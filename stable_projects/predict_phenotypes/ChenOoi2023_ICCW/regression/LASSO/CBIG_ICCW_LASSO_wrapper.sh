#!/bin/bash
#
# This function runs the LASSO regressions in ChenOoi2023.
...
#     ./CBIG_ICCW_LASSO_wrapper.sh 400 ~/storage/ICCW_replication
#
...
# 1. root_dir: The folder in which the LASSO scripts are stored
...
root_dir=${CBIG_CODE_DIR}/stable_projects/predict_phenotypes/ChenOoi2023_ICCW/regression/LASSO
...
out_dir=$results_dir/LASSO/$sample_size
...
do 
    # prepare parameters for LASSO
    LF=$out_dir/logs/Behav${behav}.log
    matlab -nodesktop -nosplash -nodisplay -r " addpath $root_dir; CBIG_ICCW_LASSO_get_parameters( \
       '$KRR_dir', '$out_dir', $behav); exit; " >> $LF 2>&1
    param_file=$out_dir/behav_${behav}/param.mat

    # run LASSO in batches of 18 folds each
