...
# Run `./CBIG_ICCW_KRR_LpOCV_workflow.sh --help` for the usage.
...
matlab -nodesktop -nosplash -nodisplay -r " addpath $root_dir; CBIG_ICCW_KRR_LpOCV_prepare_parameters( '$csv_file',\
    ...
    '$metric', '$sample_size'); exit; " >> $LF 2>&1
...
if [ "$cluster" == "none" ];then
    ...
    ${root_dir}/CBIG_ICCW_KRR_LpOCV_testloop.sh -p ${paramfile} -n ${num_test_folds} -o ${outdir}

else

    cmd="${root_dir}/CBIG_ICCW_KRR_LpOCV_testloop.sh -p ${paramfile} -n ${num_test_folds} -o ${outdir}"
...
if [ "$cluster" == "none" ];then
    ...
    do
        ${root_dir}/CBIG_ICCW_KRR_LpOCV_innerloop.sh -p ${paramfile} -t ${test_fold} -o ${outdir}
    done
    ...
    for test_fold in `seq 1 ${num_test_folds}`
    do
        cmd="${root_dir}/CBIG_ICCW_KRR_LpOCV_innerloop.sh -p ${paramfile} -t ${test_fold} -o ${outdir}"
...
usage() { echo "
NAME:
    CBIG_ICCW_KRR_LpOCV_workflow.sh
...
OPTIONAL ARGUMENTS:
    ...
    -sample_size       sample_size      : Sample size. If used, a subset of subjects will be selected for train and
                                          test. Other subjects will be ignored.
    ...
    -metric            metric           : A string stating which accuracy metric to use when optimize hyperparameters.
                                          ...
                                          `CBIG_ICCW_KRR_LpOCV_prepare_parameters.m` for more information
...
Example:
    $CBIG_CODE_DIR/stable_projects/predict_phenotypes/ChenTam2021_ICCW/KRR_LpOCV/SingleKernel/
    CBIG_ICCW_KRR_LpOCV_workflow.sh -csv_file xxx/behav_and_covariates.csv -subject_list xxx/subjects_all.txt
