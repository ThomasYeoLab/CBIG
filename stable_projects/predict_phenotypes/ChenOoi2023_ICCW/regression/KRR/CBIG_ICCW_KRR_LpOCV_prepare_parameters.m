function CBIG_ICCW_KRR_LpOCV_prepare_parameters( csv_file, subject_list, feature_file, ...
    y_list, covariate_list, FD_file, DVARS_file, outdir, outstem, num_leave_out, num_inner_folds, ...
    ker_param_file, lambda_set_file, threshold_set_file, metric, sample_size )
...
% Inputs:
%   - csv_file
%     Full path of a #subjects x #variables 
%     csv file contains behavioral data and covariates for all subjects.
%
%   - subject_list
...
sample_size = str2num(sample_size);

%% Data split
fprintf('[KRR LpOCV workflow]: leave-%d-sites-out cross-validation splits.\n',num_leave_out);
CBIG_ICCW_split_and_sample( subject_list, csv_file, 'subjectkey', ...
    'site_group_150', num_leave_out, outdir, ',', sample_size);
...
param = CBIG_ICCW_KRR_LpOCV_parseInput( sub_fold_file, fullfile(outdir, ['y' ystem '.mat']), ...
...
function param = CBIG_ICCW_KRR_LpOCV_parseInput(varargin)
