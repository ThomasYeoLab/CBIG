function [acc_corr, y_predict, y_pred_train, optimal_stats, beta] = ...
    CBIG_Elasticnet_train_test_glmnet_savebeta( feat_train, feat_test, y_train, y_test, ...
    alpha, lambda)
% function [acc_corr, y_predict, y_pred_train, optimal_stats, beta] = ...
%    CBIG_Elasticnet_train_test_glmnet_savebeta( feat_train, feat_test, y_train, y_test, ...
%    alpha, lambda)
%
% This function uses glmnet to perform an elastic-net regression and is adapted from 
% `CBIG_Elasticnet_train_test_glmnet.m` from the CBIG_repo. Based on the specified 
% regularization ratio (alpha) and regularisation hyperparameter (lambda), 
...
%
% Importantly, this function is modified so that it saves the regression weights.
%
...
%
%   - beta:
%     The #features-length vector of regression weights used for Elasticnet.
%
