function CBIG_run_Elasticnet_workflow_parallel_folds(params, start_fold, fold_number)

% function CBIG_run_Elasticnet_workflow_parallel_folds(params, start_fold, fold_number)
%
% This function uses glmnet to perform an elastic-net regression and is
% adapted from CBIG_run_Elasticnet_workflow from the CBIG_repo. Please refer
% to the CBIG_repo for the full description of the elastic-net workflow.  
% There are 2 parameters that are optimised for elasticnet: lambda (the
...
%
% Importantly, this function is modified so that a start and end test fold number
% is specified, so that different folds can be submitted to a scheduler to speed
% up processing. Secondly, this function also saves the regression weights by
% calling `CBIG_ICCW_Elasticnet_train_test_glmnet_savebeta.m`.
%
% Inputs:
...
%
%   - start_fold
%     A integer. The index of the fold to start the regression workflow from.
%
%   - fold_number
%     An integer. Number of folds to run the regression for, starting from
%     `start_fold`.
%
%  Outputs:
...
%% Preparation
project_code_dir = fullfile(getenv('CBIG_CODE_DIR'), 'stable_projects', 'predict_phenotypes', ...
    'ChenOoi2023_ICCW', 'regression', 'utilities');
...
for currfold = start_fold : start_fold+fold_number-1
    ...
    [acc_corr, y_predict, y_pred_train, optimal_statistics, beta] = ...
        CBIG_ICCW_Elasticnet_train_test_glmnet_savebeta(feat_train', feat_test', ...
        y_train, y_test, curr_alpha, curr_lambda);
    
    save([curr_param_dir '/acc_test_' params.outstem '.mat'], 'acc_corr', 'y_predict', ...
        'y_pred_train', 'optimal_statistics', 'beta');
