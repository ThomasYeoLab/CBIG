function CBIG_KRDNN_train_valid_krr(setup_file, lambda_set)
...
for i = 1:length(FSM_set)
    ...
    for m = 1:length(lambda_set)
        ...
        for j = 1:length(measures_set)
            ...
            [y_p,y_t,acc_tmp,acc_concat{i,m}] = train_valid_krr_ind(l,curr_measure,curr_FSM_type,out_dir,data_dir);
            if(j == 1)
                acc_all = acc_tmp;
            else
...
function [y_p,y_t,acc,acc_concat] = train_valid_krr_ind(lambda,measure_type,FSM_type,out_dir,data_dir)

% [y_p,y_t,acc,acc_concat] = train_valid_krr_ind(lambda,measure_type,FSM_type,out_dir,data_dir)
% 
...
dis_y_regress = measures_num_regress;

%% Load K
% K: #subject_dis x #subject_dis
load(fullfile(data_dir, 'FSM_cv', 'dis_valid', ['FSM_' FSM_type '.mat']));
num_subject = size(FSM,1);
...
index_tra = subject_split.index_tra;
index_tra(subject_split.index_tes) = [];
index_val = subject_split.index_val;
index_val(subject_split.index_tes) = [];

if(flag == 1)
    curr_FSM_train = FSM(index_tra,index_tra);
    curr_FSM_test = FSM(index_val,index_tra);
    for i = 1:length(measure_cell)
        % Training
        % alpha = (K + lambda * I)^-1 * y
        %  Nx1    NxN   1x1    NxN      Nx1
        curr_y_regress = dis_y_regress(subject_split.index_tra,i);
        curr_y_real = dis_y_regress(subject_split.index_val,i);
        nan_index = isnan(curr_y_regress);
        alpha{i} = (curr_FSM_train(~nan_index,~nan_index) + lambda*eye(sum(~nan_index))) \ curr_y_regress(~nan_index);

        % Test
        y_predict{i} = curr_FSM_test(~isnan(curr_y_real),~nan_index)*alpha{i};

        % Test real
        y_true{i} = curr_y_real(~isnan(curr_y_real));
        acc_fold{i} = corr(y_predict{i},y_true{i});
    end
    mkdir(output_dir);
...
for i = 1:length(measure_cell)
    ...
    scatter(y_predict{i},y_true{i});

    y_predict_concat = y_predict{i};
    y_true_concat = y_true{i};
    acc_all = acc_fold{i};
