function CBIG_KRDNN_generate_K_valid(setup_file)

% CBIG_KRDNN_generate_K_valid(setup_file)
% 
% This function generate kernel for validation set for kernel ridge regression
% algorithm.
%
% Inputs:
%   - setup_file
...
corr_mat_KbyS_valid = corr_mat_KbyS(:,subject_split.index_val==1);
corr_mat_KbyS_train = corr_mat_KbyS(:,subject_split.index_tra==1);

for i=1:length(FSM_set)
    curr_FSM = FSM_set{1,i};
    FSM_out = fullfile(data_dir, 'FSM_cv', 'dis_valid');
    mkdir(FSM_out);
    ...
    if (~exist(FSM_dir,'file'))
        ...
        FSM=CBIG_KRDNN_kernel_with_scale( ...
            corr_mat_KbyS_train,corr_mat_KbyS_valid, ...
            subject_split.index_tra==1,subject_split.index_val==1, ...
            curr_FSM_type,scale);
