function CBIG_KRDNN_generate_K_train(setup_file)

% CBIG_KRDNN_generate_K_train(setup_file)
% 
% This function generate kernel for training set for kernel ridge regression
% algorithm.
%
% Inputs:
%   - setup_file
...
%% remove current test_fold subjects from the fullset correlation matrix
corr_mat(:,:,subject_split.index_tes)=[];
...
for i=1:length(FSM_set)
    ...
    FSM_out = fullfile(data_dir, 'FSM', 'dis_train');
    mkdir(FSM_out);
    ...
    if (~exist(FSM_dir,'file'))
        ...
        FSM=CBIG_KRDNN_kernel_with_scale( ...
            corr_mat_KbyS,[],[],[],curr_FSM_type,scale);
