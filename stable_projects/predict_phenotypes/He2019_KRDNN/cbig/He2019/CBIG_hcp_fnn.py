...
from CBIG_model_keras import hcp_fnn
...
def train(args):
    ...
    t_overall = time.time()
    print('\nCBIG FNN for HCP with argument: ' + str(args))
    ...
    for tes_fold in range(tes_folds):
        ...
        # load data
        npz = os.path.join(args.path_data, 'fnn',
        ...
        for val_fold in range(val_folds + 1):
            ...
            if val_fold == val_folds:
                ...
                [train_x, valid_x, train_y, valid_y] = mics_train_valid_split(
                    train_valid_x, train_valid_y, fold=val_fold)

            model = hcp_fnn(train_x.shape[1], n_measure, args.n_l1, args.n_l2,
                            args.n_l3, args.dropout, args.l2_regularizer)
            optimizer = SGD(
                lr=args.lr,
    ...
    mics_hcp_log('fnn', args.out_path, **log_args)
...
def get_args():
    ...
    parser.add_argument('--l2_regularizer', type=float, default=0.02)
    parser.add_argument('--dropout', type=float, default=0.5)
    parser.add_argument('--n_l1', type=int, default=32)
    parser.add_argument('--n_l2', type=int, default=16)
    parser.add_argument('--n_l3', type=int, default=8)
