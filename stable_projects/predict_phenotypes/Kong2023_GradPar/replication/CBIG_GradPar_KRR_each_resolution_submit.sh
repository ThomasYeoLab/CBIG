#!/bin/bash
#
# This script is used to submit jobs for each method and resolution for KRR.
# To save time, here we only submit jobs for Kong2021 in HCP dataset with 100 and 200 resolutions
# and for PrincipalGrad in ABCD dataset with 1 gradient. We will only run 3 splits for Kong2021.
# 
# Example:
#     sh CBIG_GradPar_KRR_each_resolution_submit.sh ~/storage/Temporary/CBIG_GradPar_replication
#
...
####################################################
# Submit jobs for each method and resolution for KRR
####################################################
# To save time, here we only submit jobs for Kong2021 in HCP dataset with 100 and 200 resolutions
# and for PrincipalGrad in ABCD dataset with 1 gradient. We will only run 3 splits for Kong2021.
#
...
for (( res=100; res<=200; res+=100 )); do
    ...
    for (( nsplit=1; nsplit<=3; nsplit+=1 )); do
        ...
        cmd="$cmd \"clear;clc;close all;CBIG_GradPar_KRR_each_resolution_HCP_wrapper \
...
# PrincipalGrad for ABCD dataset
# Can set <curr_project_name> to Kong2021, Schaefer2018, PrincipalGrad, LocalGrad, sICA for different methods
curr_project_name='PrincipalGrad'
# loop resolution 1:
# Can change the <res> range for each method
for res in 1; do
    ...
    cmd="cd ${CBIG_CODE_DIR}/stable_projects/predict_phenotypes/Kong2023_GradPar/replication;"
    cmd="$cmd matlab -nosplash -nodisplay -nodesktop -r "
    cmd="$cmd \"clear;clc;close all;CBIG_GradPar_KRR_each_resolution_ABCD_wrapper \
