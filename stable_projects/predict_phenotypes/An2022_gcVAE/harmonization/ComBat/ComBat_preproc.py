...
def ComBat_preproc(data_path,
    ...
    Args:
        data_path (str): Path for input csvs
        harm_output_path (str): Path for preprocessed csvs
        covariates (list): List of names for covariates
        prefix (str): Prefix for saving
        train_file (str): Name for training csv
        val_file (str): Name for validation csv
        test_files (list): List of name for test csvs
...
def ComBat_preproc_ff_covariates(train_df, test_dfs, covariates):
    ...
    Args:
        train_df (class DataFrame): Training dataframe
        test_dfs (list): List of testing dataframes
        covariates (list): List of names for covariates
    """
    # train
...
def ComBat_preproc_extract_ROI(dfs, ROIs, save_names, output_path):
    ...
    Args:
        ...
        ROIs (List): Features to harmonize
        save_names (List): List of names for saving output
...
def ComBat_preproc_extract_covariates(dfs, covariates, save_names,
    ...
    Args:
        dfs (List): List of dataframes
        covariates (List): Covariates used to in harmonization
        save_names (_type_): List of names for saving output
        output_path (_type_): Path for saving output
    """
    assert len(dfs) == len(save_names), 'dfs is not matched with save_names!'
    for df, save_name in zip(dfs, save_names):
