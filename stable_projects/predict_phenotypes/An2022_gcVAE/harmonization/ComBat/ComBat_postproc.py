...
def ComBat_postproc(data_path, output_path, harm_files, nb_folds=10):
    """
    Postprocess harmonized data

    Args:
        data_path (str): Path for input csv
        output_path (str): Path for saving output
        harm_files (list): List of names for harmonzied dataframes
...
def ComBat_potproc_single_csv(fold_data_path, fold_output_path, features,
    ...
    Args:
        fold_data_path (str): Path for <fold> input csv
        fold_output_path (str): Path for <fold> saving output
        features (list): Features to harmonzie
        harm_csvs (list): List of names for harmonzied dataframes
        origin_csvs (list): List of names for original dataframes
    """
    if len(origin_csvs) == 2:
        # train_val
        harm_train_val_ROI_df = csvs2df(
            fold_output_path, harm_csvs, index_col=0)
        ...
        harm_test_df = replace_with_harmed_ROI(
            test_df, harm_test_ROI_df, features, isTranspose=True)
