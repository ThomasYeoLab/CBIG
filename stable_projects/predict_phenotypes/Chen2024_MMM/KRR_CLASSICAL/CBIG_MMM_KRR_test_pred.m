function [acc, pred_stats] = CBIG_MMM_KRR_test_pred(test_fold, sub_fold, ...
    data_dir, y_resid_stem, with_bias, kernel, ker_param, lambda_set, threshold_set)

% [acc, pred_stats] = CBIG_MMM_KRR_test_pred( test_fold, sub_fold, ...
...
% all target variables with corresponding hyperparameters combination,
...
%     entry of the cell array is an #TestSubject x 1 vector storing the
%     predicted values of all the test subjects.
%
%   - y_t
%     A cell array with dimension of #kernels x #lambda x #thresholds. Each
%     entry of the cell array is an #TestSubject x 1 vector storing the
...
if(length(sub_fold)>1)
    ...
    test_ind = sub_fold(test_fold).fold_index==0;
    train_ind_y = train_ind;
    test_ind_y = test_ind;
else
    train_ind = find(sub_fold.fold_index==0);
    test_ind = find(sub_fold.fold_index==0);
    train_ind_y = train_ind;
    test_ind_y = test_ind;
end
