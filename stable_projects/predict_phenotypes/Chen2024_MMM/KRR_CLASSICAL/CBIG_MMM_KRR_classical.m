function CBIG_MMM_KRR_classical(CBIG_CODE_DIR, subject_list, phe_csv, FC_file, output_dir, n_rng, phe, k,...
    flag_pred_train, prefix)

% CBIG_MMM_KRR_classical(CBIG_CODE_DIR, subject_list, phe_csv, FC_file, output_dir, n_rng, phe, k, flag_pred_train)
% 
% This function runs the kernel ridge regression algorithm to one of phenotypes.
%
% Inputs:
%   - CBIG_CODE_DIR
...
% 
%   - subject_list
%     Full path of subject list. It should be a txt file that contains #subject 
%     of lines, where each line is the subject id of 1 subject.
% 
%   - phe_csv
%     Full path of the phenotypes csv. It should contains the phenotypes of 
%     all subjects.
%  
%   - FC_file
...
% 
%   - phe
%     phenotypes (non-brain-imaging phenotypes) to perform the
...
%   
%   - prefix
%     str of the prefix for the dataset (e.g, 'HCP', 'HCPA')
% 
% Written by Pansheng Chen and CBIG under MIT license: https://github.com/ThomasYeoLab/CBIG/blob/master/LICENSE.md
    ...
    for rng_num = 1:n_rng
        ...
        data_dir = fullfile(data_output_dir, [prefix '_' phe '_k_' k '_rng_num_' num2str(rng_num)]);
        ...
        % split data
        CBIG_MMM_KRR_classical_split_subject(data_dir, subject_list, rng_num + rng_offset, str2num(k), param.y, prefix)
        temp = load(fullfile(data_dir, [prefix '_subject_split.mat']));
        ...
        while size(unique(y_train), 1) == 1 || max(hist(y_train,unique(y_train)))/size(y_train, 1) >= thr_y_znorm
            ...
            CBIG_MMM_KRR_classical_split_subject(data_dir, subject_list,...
                rng_num + rng_offset, str2num(k), param.y, prefix)
            temp = load(fullfile(data_dir, [prefix '_subject_split.mat']));
            y_train = y(temp.sub_fold.fold_index == 0);
        end
        param.sub_fold = temp.sub_fold;

        % if phenotype is not 31-0.0 (sex), do z-norm
        if ~strcmp(phe, '31-0.0') && ~strcmp(phe, 'sex')
        ...
        if flag_pred_train
            ...
            CBIG_MMM_KRR_test_pred(test_fold, tmp_fold, ...
