function CBIG_MMP_ABCD_plot_importance_wrapper(int_dir)
% CBIG_MMP_ABCD_plot_importance_wrapper
% This is a wrapper function that plots the feature importance for all 
% all KRR models for the ABCD.
%
...
for n = 1:length(modalities)
    ...
    for m = 1:length(curr_mod)
        ...
        % 7 network ordering
        if strcmp(modalities{n}, 'tractography') | strcmp(modalities{n}, 'fmri')
            network_rearr = 17;
        else 
            network_rearr = 0;
        end
        load(fullfile(int_dir, curr_outstem , 'cov_mat.mat'))
        mod_mean_importance(m,:,:) = modality_plot(modalities{n}, ...
            cov_mat, int_dir, curr_outstem, network_rearr); 
    end
...
pers_idx = 39;
mh_idx = 38; 
prefixes = {'cog' 'pers' 'mh'};

% calculate mean importance
mean_importance = squeeze(mean(cov_mat_mean,1));
imp_cog = mean_importance(:,cog_idx);
imp_pers = mean_importance(:,pers_idx);
imp_mh = mean_importance(:,mh_idx);
...
switch modality
    case "t1"
        ...
        % personality
        prefix = prefixes{2};
        CBIG_MMP_plot_cortical(imp_pers, fullfile(int_dir, curr_outstem), ...
            prefix)
        % mental health
        prefix = prefixes{3};
        CBIG_MMP_plot_cortical(imp_mh, fullfile(int_dir, curr_outstem), ...
            prefix)
    case "tbss"
        fprintf("Plot TBSS\n")
        ...
        skeleton_mask = fullfile('/home', 'leon_ooi', 'storage', 'Multimodal_prediction_project', ...
            'replication', 'ABCD', 'input', 'ABCD_processed_data', 'mean_FA_skeleton_mask.nii.gz');
        % cognition
        prefix = prefixes{1};
        ...
        % personality
        prefix = prefixes{2};
        CBIG_MMP_plot_tbss(imp_pers, fullfile(int_dir, curr_outstem), prefix, ...
            skeleton_mask)
        % mental health
        prefix = prefixes{3};
        CBIG_MMP_plot_tbss(imp_mh, fullfile(int_dir, curr_outstem), prefix, ...
            skeleton_mask)
    case {"tractography", "fmri"}
        ...
        % personality
        prefix = prefixes{2};
        CBIG_MMP_plot_ROI2ROI(imp_pers, fullfile(int_dir, curr_outstem), ...
            prefix);
        % mental health
        prefix = prefixes{3};
        CBIG_MMP_plot_ROI2ROI(imp_mh, fullfile(int_dir, curr_outstem), ...
            prefix);
end
end