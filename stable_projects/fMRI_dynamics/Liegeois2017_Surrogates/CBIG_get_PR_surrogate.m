function surr = CBIG_get_PR_surrogate(TC , n_surr)

% surr = CBIG_get_PR_surrogate(TC , n_surr)
...
% r1     = corrcoef(x) ;
% surr   = CBIG_get_PR_surrogate(x,2);
% r2     = corrcoef(surr(:,:,2)); % Check that the correlation is preserved
% norm(r1-r2); 

% Written by Raphael Liegeois and CBIG (based on previous work by Carlos Gias) under MIT license: https://github.com/ThomasYeoLab/CBIG/blob/master/LICENSE.md
