...
import numpy as np
import os
from CBIG_DELSSOME_Hopf_optimizer import Hopf_Validator, Hopf_Tester, train_help_function
...
def optimize_body(mode,
    ...
    Args:
        mode (str): choices from ['train', 'validation', 'test']
    ...
    if mode == 'train':
        ...
        mfm_validator = Hopf_Validator(config, save_param_dir, val_dir)
        mfm_validator.val_best_parameters_all_epochs(sc_euler, fc_emp, fcd_cdf_emp, num_epochs, seed=seed)

    elif mode == 'test':
        # Only this seed's validation results are pooled. To pick the best parameters
        # across all seeds instead, widen the range to range(1, seed_nbr + 1).
        ...
        mfm_tester = Hopf_Tester(config, val_dirs, test_dir, trained_epochs=num_epochs)
...
def optimize_main(args):
    ...
    for seed_nbr in range(args.start_seed_nbr, last_seed + 1):
        ...
        if args.mode in ['train', 'validation', 'test']:
            ...
            # test applies the best checkpoint and saves final metrics.
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser('Hopf settings')
    parser.add_argument(
        '--mode',
        type=str,
        choices=['train_val_test', 'train', 'validation', 'test'],
        default='train',
    )
    ...
    parser.add_argument('--num_epochs', type=int, default=2, help='The total number of CMA-ES training epochs')
