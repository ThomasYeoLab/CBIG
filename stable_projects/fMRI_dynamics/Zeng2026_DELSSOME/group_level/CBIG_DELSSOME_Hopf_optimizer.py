...
from models.CBIG_DELSSOME_Hopf import HopfModel

from utils.CBIG_DELSSOME_pFIC_utils import parameterize_myelin_rsfc_Hopf, FC_calculate, FCD_calculate, \
...
def train_help_function(config,
    ...
    for i in range(trials):
        state = mfm_trainer.train_Hopf(seed=seed, next_epoch=next_epoch)
...
class CMAES_Trainer:
    ...
    def __init__(self, config, myelin, RSFC_gradient, sc_mat, fc_emp, fcd_cdf_emp, save_param_dir, num_epoch,
        ...
        self.simulation_period = float(system_parameters['simulation_period'])
        self.burn_in_period = float(system_parameters['t_pre'])
        ...
        self.warmup_period = int(system_parameters['warmup'])
    ...
    def get_parameters(self, param_10):
        ...
        """
        return parameterize_myelin_rsfc_Hopf(self.myelin, self.RSFC_gradient, param_10)

    def hopf_model_loss_dl(self, parameter, epoch):
        """
        ...
        with torch.no_grad():
            ...
            a = valid_parameter[:, 0:self.N]
            omega = valid_parameter[:, self.N:2 * self.N]
            ...
            parameter_sets = torch.stack((a, omega, sigma), dim=-1)  # [n_param, n_roi, 3]
    ...
    def hopf_model_loss_euler(self, parameter, epoch):
        """
        ...
        hopf_model = HopfModel(self.config, parameter_repeat, self.sc_euler, dt=self.dt)
        time_series, valid_M_mask = hopf_model.simulate(simulation_period=self.simulation_period,
        ...
        time_series = time_series.view(self.N, self.param_dup, self.param_sets, -1).transpose(1, 2)
        ...
        for i in range(self.param_sets):
            ...
            if mask_this_param.any():
                ...
                bold_this_param = time_series[:, i, mask_this_param, :]  # [N, 1/2/3/param_dup, t_for_bold]
        ...
        # For development, also save time series
        save_dict = {
            'parameter': parameter,
    ...
    def train_Hopf(self, seed=None, next_epoch=0):
        ...
        search_range[0:N, 0] = -1  # search range for a
        search_range[0:N, 1] = 0
        search_range[N:2 * N, 0] = 0  # search range for omega
        search_range[N:2 * N, 1] = 12.56
        ...
        if next_epoch == 0:
            ...
            # initialization, k = 0
            m_k, sigma_k, cov_k, p_sigma_k, p_c_k = self._init_CMA_ES(search_range)
        ...
        for k in range(next_epoch, self.epochs):
            ...
            if k in self.epoch_range:
                epoch_res = self._train_one_epoch(k, m_k, sigma_k, cov_k, p_sigma_k, p_c_k)
    ...
    def _train_one_epoch(self, k, m_k, sigma_k, cov_k, p_sigma_k, p_c_k):
        param_10_k, parameter_k = self._sample_valid_parameters(m_k, sigma_k**2 * cov_k, self.search_range)
        if param_10_k is None or parameter_k is None:
            print("Sampling failed!")
            return None

        if k in self.dl_epoch_range:
            loss_k, index_k = self.hopf_model_loss_dl(parameter_k, k)
        elif k in self.euler_epoch_range:
            loss_k, index_k = self.hopf_model_loss_euler(parameter_k, k)
        else:
        ...
        m_kp1, sigma_kp1, cov_kp1, p_sigma_kp1, p_c_kp1 = self._update_CMA_ES(select_params, loss_k, m_k, sigma_k,
                                                                              cov_k, p_sigma_k, p_c_k, k)
        return m_kp1, sigma_kp1, cov_kp1, p_sigma_kp1, p_c_kp1

    def _init_CMA_ES(self, search_range):
        """
        ...
        start_point_a = torch.matmul(self.pinv_concat_mat, init_parameters[0:N]).squeeze()
        start_point_omega = torch.matmul(self.pinv_concat_mat, init_parameters[N:2 * N]).squeeze()
        ...
        m_0[0:3] = start_point_a
        m_0[3:6] = start_point_omega
        ...
        Lambda_ini[0:3] = start_point_a[0]
        Lambda_ini[3:6] = start_point_omega[0]
        Lambda_ini[6] = 0.4
        Lambda_ini[7:] = 0.0005
    ...
    def _update_CMA_ES(self, select_params, loss_k, m_k, sigma_k, cov_k, p_sigma_k, p_c_k, k):
        """
...
class Hopf_Validator:
    ...
    def __init__(self, config, save_param_dir, val_save_dir, val_by_dl=False):
        ...
        if self.val_by_dl:
            ...
            self.dl_predictor = MultiModalCostPredictor(n_regions=self.N, d_param=self.parameters_dim, d=d_transformer)
            self.dl_predictor.load_state_dict(
                torch.load(predictor_save_path, map_location=torch.device(self.device))['model_state_dict'])
    ...
    def _val_best_parameters_all_epochs_by_euler(self, parameter_all_epochs, sc_euler, fc_emp, fcd_cdf_emp, num_epochs):
        ...
        hopf_model = HopfModel(self.config, parameter_repeat, sc_euler, dt=self.dt)
        time_series, valid_M_mask = hopf_model.simulate(simulation_period=self.simulate_time,
                                                        burn_in_period=self.burn_in_time,
                                                        TR=self.TR,
                                                        warmup_period=self.warm_up_t)

        time_series = time_series.view(self.N, self.param_dup, num_epochs, -1).transpose(1, 2)
        # [N, param_sets, param_dup, t_for_bold]
...
class Hopf_Tester:
    ...
    def __init__(self, config, val_dirs, test_dir, trained_epochs):
        ...
        print(f"""Hopf testing process successfully initialized.
    ...
    def select_best_from_val(self):
        """Need Modify if the number of losses is not 3.
