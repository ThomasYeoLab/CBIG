...
class PredictorTrainer:
    ...
    def train(self, need_early_stop=False):
        ...
        for epoch in range(self.pre_epoch, self.epochs):
            ...
            if need_early_stop:
                # Halt if validation loss has been monotonically increasing for 3 consecutive epochs
                # (checked after the first 5 warm-up epochs).
    ...
    def _train_one_epoch_by_all(self, epoch):
        ...
        for i, (parameter, ground_loss, sc_mat, emp_fc, emp_fcd) in enumerate(self.train_loader):
            if parameter.shape[0] == 1:
                # Skip single-sample batches: batch normalization produces undefined statistics with batch size 1.
                continue
    ...
    def _test_one_set(self, group_nbr):
        ...
        with torch.no_grad():
            ...
            if self.need_loss_figure:
                # save the pred_loss and ground_loss first
                torch.save({
                    'pred_loss': pred_loss,
...
def train_main(config_path, seed=None):
    ...
    trainer = PredictorTrainer(config, seed=seed)
    trainer.train()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='DELSSOME predictor trainer')
    parser.add_argument('--config_path',
                        type=str,
                        default='../examples/config/example_predictor_train.ini',
