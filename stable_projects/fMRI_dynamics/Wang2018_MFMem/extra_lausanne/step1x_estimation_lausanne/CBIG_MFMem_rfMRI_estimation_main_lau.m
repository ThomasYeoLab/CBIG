...
%
% REFERENCE LIST:
% [1](Deco 2013), Resting-state functional connectivity emerges from structurally and dynamically shaped slow 
%   linear fluctuations.
...
% [8](Wang, unpublished), Short-term plasticity of laminar synaptic connections in Alzheimer's disease - An 
%   MEG study using Bayesian inversion.
%
...
%load FC and SC
load(fullfile(data_dir,'FCSC_Schaefer.mat'),'FC_training','SC_training'); 
...
%-----------------------------------------------------------
% set up prior for G(globle scaling of SC), w(self-connection strength/excitatory),Sigma(noise level),
% Io(background input)
...
lembda_step_save = zeros(n,EstimationMaxStep);    
rrr = zeros(1,EstimationMaxStep);                 
rrr_z  = zeros(1,EstimationMaxStep);              
Para_E_step_save = zeros(p,EstimationMaxStep);   
...
%---------------------  
    ...
    funcP = @(Para_E) CBIG_mfm_rfMRI_nsolver_eul_sto(Para_E,Prior_E,SC,y,FC_mask,Nstate,14.4,0.72,0);
    funcA = @(A) CBIG_mfm_rfMRI_nsolver_eul_sto(A,Prior_E,SC,y,FC_mask,Nstate,14.4,0.72,1);
    ...
    %-------------------------------------------------------------------- 
     parfor i = 1:2*p
        if i <= p
            disp('running JF')
            JFK(:,i) = CBIG_mfm_rfMRI_diff_P1(funcA,A,h_output,i); % {nT x p}
        else
            disp('running Jk')
            JFK(:,i) = CBIG_mfm_rfMRI_diff_PC1(funcA,A,i-p);
        end
     end
    %-------------------------------------------------------------------- 
...
%          disp(mi)
        ...
        g_rest = 0.5*bsxfun(@times,r',inv_DiagCe)*exp(lembda(i))*CBIG_mfm_rfMRI_matrixQ(i,n,T); 
        ...
        for i = 1:n
            ...
            H(i,j) = 0.5*exp(lembda(i))*exp(lembda(j))*CBIG_mfm_rfMRI_Trace_AXB(Pij,Pji);
            end
        end
...
save( [save_dir '/Schaefer100_' num2str(saved_date(1)) num2str(saved_date(2)) ...
    num2str(saved_date(3))],'Para_E','rrr_z_max');
save('save_all_for_check')
poolobj = gcp('nocreate');
delete(poolobj);
rmpath(lib_dir);
