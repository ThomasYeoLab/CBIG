function CBIG_MFMem_rfMRI_DrawBarchart_I_Desikan68_into_Yeo7_fsaverage5()

%--------------------------------------------------------------------------
% CBIG_MFMem_rfMRI_DrawBarchart_I_Desikan68_into_Yeo7_fsaverage5
%
% show mean value of I in Yeo's 7-network
% what to do?
% 1) give the value of I to each voxel, according to the Desikan labels, mapping I from Desikan68 to voxel
% 2) caculate the value for Yeo 52 components, mapping I from each voxel to Yeo 52 components
% 3) caculate the value for Yeo 7-networks, mapping I from Yeo 52 components to 7-networks
%
...
%% give the value of I to each voxel, according to the Desikan labels
%generate new lh_Ivector_36, rh_Ivector_36
lh_Ivector_36 = zeros(36,1);
rh_Ivector_36 = zeros(36,1);
lh_Ivector_36([2:4 6:36]) = Ivector(1:34);  %Ivector has no No.1 unknown, and No.5 corpus callsum
rh_Ivector_36([2:4 6:36]) = Ivector(35:68);

%give the value of I to each voxel, according to the Desikan
lh_vox_value = zeros(10242,1);
rh_vox_value = zeros(10242,1);
for i = 1:10242
    lh_vox_value(i) = lh_Ivector_36(lh_label_desikan(i));
end
for i = 1:10242
    rh_vox_value(i) = rh_Ivector_36(rh_label_desikan(i));
end
...
% assign 52 components to 7 network
...
%% caculate I mean and error bar in 7-networks 
for i = 1:7
...
figure
bar(regions_network7_value_mean,'facecolor','w','edgecolor','k');
ylim([0.18 0.38]);
hold on
errorbar([1:7], regions_network7_value_mean, regions_network7_errorbar,'+k');
set(gca,'xTick',[1:7],'xTicklabel', Name7,'yTick',[0.18:0.04:0.38])
set(gca,'Fontsize',12,'Fontname','arial','TickDir','out')
hold off
title('Subcortical input I')
