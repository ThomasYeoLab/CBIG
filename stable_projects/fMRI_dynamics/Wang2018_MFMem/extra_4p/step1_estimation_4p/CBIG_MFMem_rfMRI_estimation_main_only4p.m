...
% is inside function
%    ->lib/CBIG_MFMem_rfMRI_nsolver_eul_sto.m
%
...
p = 2 + 2; %number of estimated parameter
Prior_E = zeros(p,1);
...
%---------------------  
    ...
    funcP = @(Para_E) CBIG_MFMem_rfMRI_nsolver_eul_sto_4p(Para_E,Prior_E,SC,y,FC_mask,Nstate,14.4,0.72,0);
    funcA = @(A) CBIG_MFMem_rfMRI_nsolver_eul_sto_4p(A,Prior_E,SC,y,FC_mask,Nstate,14.4,0.72,1);
...
save( [save_dir '/Estimated_Parameter_for_only_4p_' num2str(saved_date(1)) num2str(saved_date(2)) num2str(saved_date(3))],'Para_E','rrr_z_max');
save('save_all_for_check_4parameter')
poolobj = gcp('nocreate');
delete(poolobj);
rmpath(lib_dir);
