...
def CBIG_mfm_optimization_main(parser, process_id, random_seed):
    ...
    constant (in total, 10 parameters).
    This function utilizes python multiprocessing to maximize the GPU usage.
    -Input:
    ...
    -Output:
        ...
        -training.csv: optimized parameters and associated costs.
            Each row corresponds to 1 CMAES epoch, the first 7 numbers are
            the linear coeff of wEE, linear coeff of wEI, G and linear coeff
            of sigma. The last 4 numbers of FC correlation loss, FC L1 loss,
    ...
    [Terminology] 1.'Model parameter/regional parameter/parameter' refers to
                  ...
                  2.'Linear coefficients' refer to the linear coefficients of
                    model parameter. For example wEE = a + b*myelin , here,
                    a and b are the linear coefficients. Together with G (
                    global scaling constant), these are the variables that
                    are directly optimized during CMAES.
    ...
    N = 3 * (num_myelin_component + 1) + 1
    # number of linear coeffs associated with each parameter
    N_p = num_myelin_component + 1
    ...
    start_point_wEE, concat_matrix = get_init(myelin_data, init_para[0:n_node])
    start_point_wEI, concat_matrix = get_init(myelin_data,
                                              init_para[n_node:n_node * 2])
    start_point_sigma, concat_matrix = get_init(myelin_data,
...
def get_init(myelin, init_para):
    ...
    -Output:
        ...
        -concat_matrix: a horizontally concatenated matrix consisted of a bias
            term and myelin data

    """

    n_node = myelin.shape[0]
    concat_matrix = np.vstack((np.ones(n_node),
                               myelin.T)).T  # bias term, myelin
...
if __name__ == "__main__":
    ...
    if len(random_seed) != num_thread:
        raise Exception(
            "The number of multiprocessing threads does not match the "
