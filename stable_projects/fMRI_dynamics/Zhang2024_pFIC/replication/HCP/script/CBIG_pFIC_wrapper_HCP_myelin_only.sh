...
# are only parameterized by T1w/T2w ratio. This script does not require any input argument.
# Written by Shaoshi Zhang and CBIG under MIT license: https://github.com/ThomasYeoLab/CBIG/blob/master/LICENSE.md

config=../config/HCP_myelin_only.ini
module load cuda/11.7
source activate pFIC

# training
python -W ignore CBIG_pFIC_training_HCP_myelin_only.py $config
python -W ignore CBIG_pFIC_transpose_matrix.py $config

# validation
python -W ignore CBIG_pFIC_validation_HCP_myelin_only.py $config
...
user_output_path=`realpath ../test_myelin_only`
reference_output_path=`realpath ../reference_output/test_myelin_only`
