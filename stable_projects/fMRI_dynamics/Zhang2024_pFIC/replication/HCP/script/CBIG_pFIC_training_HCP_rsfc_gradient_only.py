...
def CBIG_mfm_optimization_main(parser, process_id, random_seed):
    ...
    [Terminology] 1.'Model parameter/regional parameter/parameter'
                  ...
                  2.'Linear coefficients' refer to the linear coefficients
                    of model parameter. For example wEE = a +
                    b*RSFC_gradient, here, a, b, and c are the linear
                    coefficients. Together with G (global scaling constant),
                    these are the variables that are directly optimized
                    during CMAES.
    ...
    N = 3 * (num_gradient_component + 1) + 1
    # number of linear coeffs associated with each parameter
    N_p = num_gradient_component + 1
    ...
    start_point_wEE, concat_matrix = get_init(gradient_data,
                                              init_para[0:n_node])
    start_point_wEI, concat_matrix = get_init(gradient_data,
                                              init_para[n_node:n_node * 2])
    start_point_sigma, concat_matrix = get_init(gradient_data,
...
def get_init(gradient, init_para):
    """
    ...
    -Output:
        ...
        -concat_matrix: a horizontally concatenated matrix consisted of a bias
            term and RSFC gradient data

    """

    n_node = gradient.shape[0]
    concat_matrix = np.vstack((np.ones(n_node),
                               gradient.T)).T  # bias term, RSFC gradient
