...
def CBIG_mfm_test_main(parser):
    ...
    validation cost (referred as solutions). By default, forward simulation
    is run 1000 times for each solution
    (each with a different noise instantiation) and the results are
    averaged every 10 times (so there are 100 costs in the end).
    The difference between this script and the CBIG_pFIC_test.py script is
    how the (dis)similarity constraint is implemented. Here, the distance
    between 2 sets of parameters is meausred using L1 distance instead of
    Pearson correlation.
    -Input:
    ...
    -Output:
        ...
        -solution.csv: test costs of each repetition
            (each column represents an averaged FC corr cost, FC L1 cost
            and FCD cost across 10 simulations, by default)
        -test_all.csv: a compiled file containing the training,
            validation and test costs for each solution. Each column
            corresponds to a solution. The first row corresponds to
            the process-id, the second row is the epoch number, the 3rd to
            6th rows are training costs, 7th to 10th are validation costs,
            and 11th to 14th are the test costs (averaged across all
            repetitions).
    ...
    while i < n_solution and p < vali_raw_all.shape[1]:
        L1_t = np.zeros(n_solution, dtype=bool)
        L1_tr = np.zeros(n_solution)
        for j in range(n_solution):
            wEE_L1 = np.sum(
                np.abs((vali_sel[10:10 + n_node, j:j + 1].T -
                        vali_sort_all[10:10 + n_node, p:p + 1].T)))
            L1_tr[j] = wEE_L1

        for k in range(n_solution):
            L1_t[k] = (L1_tr[k] < 100 * np.exp(-0.5 * i)).all()
