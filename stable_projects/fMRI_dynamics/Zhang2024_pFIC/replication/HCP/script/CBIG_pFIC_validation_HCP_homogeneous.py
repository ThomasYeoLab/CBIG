...
def CBIG_mfm_validation_main(parser, process_id):
    ...
    -Output:
        ...
        -S_E.mat: temporal average of simulated
            excitatory synaptic gating variable
        -S_I.mat: temporal average of simulated
            inhibitory synaptic gating variable
        -r_E.mat: temporal average of simulated excitatory firing rate
        [NOTE]: To save space, only the last 50 epoch's
            parameter's TC, S_E, S_I, r_E are saved as .mat
        -validation.csv: validation cost associated with
            each set of parameters. Each column corresponds to 1 CMAES
            epoch. The first 4 rows are training FC corr loss, training
            FC L1 loss, training FCD KS loss and training total loss. The
            5th to 8th rows are validation FC corr loss, validation FC L1
            loss, validation FCD KS loss and validation total loss.
            The rest of the rows correspond to #ROI regional wEE,
            #ROI regional wEI, G, and #ROI regional sigma.
    """
    ...
    wEI = np.tile(x_mass[1, :], (n_node, 1))
    sigma = np.tile(x_mass[3, :], (n_node, 1))
    G = np.expand_dims(x_mass[2, :], 0)
    arx_mass = np.concatenate((wEI, wEI, G, sigma), 0)
