...
def CBIG_mfm_optimization_main(parser, process_id, random_seed):
    ...
    output_path = parser.get('training', 'output_path', vars=os.environ)
    output_file = parser['training']['output_file'] + '_' + str(
        process_id) + '.csv'
    FCD = parser.get('training', 'FCD_training', vars=os.environ)
    SC = parser.get('training', 'SC_training', vars=os.environ)
    FC = parser.get('training', 'FC_training', vars=os.environ)
    ...
    myelin = parser.get('training', 'myelin_training', vars=os.environ)
    rsfc_gradient = parser.get(
        'training', 'RSFC_gradient_training', vars=os.environ)
