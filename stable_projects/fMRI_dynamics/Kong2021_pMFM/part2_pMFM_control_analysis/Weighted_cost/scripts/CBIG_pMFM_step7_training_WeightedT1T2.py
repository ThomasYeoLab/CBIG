...
def CBIG_mfm_optimization_desikan_main(gpu_index=0, random_seed=1, weight=1):
    '''
    ...
    Args:
        ...
        weight:         weight of FC cost
    Returns:
        None
    '''

    output_path = '../output/weight_' + str(weight) + '/T1T2/training'
    ...
    while countloop < maxloop:
        ...
        C = (1 - c1 - cmu) * C + c1 * (np.outer(pc, pc) + (1 - hsig) * cc * \
             (2 - cc) * C) + cmu * np.dot(artmp, np.dot(np.diag(weights), artmp.T))
...
if __name__ == "__main__":
    CBIG_mfm_optimization_desikan_main(gpu_index=0, random_seed=1, weight=1)
