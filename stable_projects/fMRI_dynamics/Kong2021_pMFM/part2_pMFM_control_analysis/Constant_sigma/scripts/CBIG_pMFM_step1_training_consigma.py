...
def CBIG_mfm_optimization_desikan_main(gpu_index=0, random_seed=1):
    ...
    xmean[2 * (2 * highest_order + 1) + 1:N] = init_para[2 * n_node + 1]
    ...
    while countloop < maxloop:
        ...
        while j < Lambda:
            ...
            input_para[2 * n_node + 1:, j] = arx[2 * (2 * highest_order + 1) +
                                                 1, j]
        ...
        input_para_m[0:n_node * 2 + 1, :] = input_para[0:n_node * 2 + 1, :]
        input_para_m[n_node * 2 + 1:n_node * 3 + 1, :] = np.repeat(
            input_para[n_node * 2 + 1:n_node * 2 + 2, :], n_node, 0)
