...
def get_init(myelin_data, highest_order, init_para):
    ...
    for i in range(highest_order + 1):
        bmatrix[:, i] = myelin_data**(i)
    para = np.linalg.inv(bmatrix.T @ bmatrix) @ bmatrix.T @ init_para
    return para, bmatrix


def CBIG_mfm_optimization_desikan_main(gpu_index=0, random_seed=1, subject=1):
    '''
    ...
    subject_path = '../../../input/Desikan_input/individual_input/subject' + str(
        subject) + '/'
    output_path = '../output/subject' + str(subject) + '/T1T2/training/'
    ...
    gradient_data = fc.csv_matrix_read(
        '../../../input/Desikan_input/myelin.csv')
    ...
    init_para = fc.csv_matrix_read('initialization_points.csv')
    init_para = init_para[11:, random_seed - 1]
...
if __name__ == "__main__":
    CBIG_mfm_optimization_desikan_main(gpu_index=0, random_seed=1, subject=1)
