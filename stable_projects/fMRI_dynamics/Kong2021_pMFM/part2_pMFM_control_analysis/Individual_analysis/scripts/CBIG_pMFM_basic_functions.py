...
def CBIG_mfm_single_simulation_mech(parameter, sc_mat, t_epochlong, d_t=0.01):
    '''
    Function used to generate the simulated BOLD signal using mean field model and hemodynamic model
    Each parameter set is ussed to simulated one time
    Args:
...
def CBIG_mfm_mech_coef(y_t, parameter, sc_mat):
    '''
    This function is to calcualte the derivatives of synaptic gating variable S
    Args:
        y_t:        N*M matrix represents synaptic gating variable
    ...
    f_x = torch.exp(-d * (a * x - b))
    f1_x = -d * a * f_x
    f2_x = -d * a * f1_x
    H = (a * x - b) / (1 - f_x)
    H1 = a / (1 - f_x) + (a * x - b) * f1_x / (1 - f_x)**2
    H2_1 = -f1_x / (1 - f_x)**2
    H2_2 = ((a * f1_x + (a * x - b) * f2_x) * (1 - f_x)**2 + 2 *
            (a * x - b) * f1_x**2 * (1 - f_x)) / (1 - f_x)**4
    H2 = H2_1 + H2_2
...
def CBIG_FCDKSstat_multi_upgrade(emp_ks, bold_d, n_dup):
    '''
    This function is to calculate the FCD KS statistics cost for multiple simulation
    BOLD signal results
    Args:
    ...
    for b in range(batch_num):
        bold_temp = bold_d[:, b * sub_num:(b + 1) * sub_num, :]
        bold_sw = torch.zeros((n_nodes, window_size, time_lengh, sub_num))

        for i in range(0, time_lengh):
            bold_sw[:, :, i, :] = torch.transpose(
                bold_temp[:, :, i:i + window_size], 1, 2)

        bold_sw_var = torch.zeros((n_nodes, time_lengh, sub_num))
        for i in range(n_nodes):
            bold_node = bold_sw[i, :, :, :]
            bold_node_mean = torch.mean(bold_node, 0)
            bold_sw[i, :, :, :] = bold_node - torch.unsqueeze(
                bold_node_mean, 0).repeat(window_size, 1, 1)
            bold_sw_var[i, :, :] = torch.std(bold_sw[i, :, :, :], 0)

        fc_batch = torch.zeros((int(n_nodes * (n_nodes - 1) / 2), time_lengh,
                                sub_num))
        fc_count = 0
        for i in range(n_nodes):
            for j in range(i):
                fc_step1 = torch.sum(bold_sw[i, :, :, :] * bold_sw[j, :, :, :],
                                     0)
                fc_batch[fc_count, :, :] = fc_step1 / (
                    bold_sw_var[i, :, :] * bold_sw_var[j, :, :])
                fc_count += 1

        fc_batch_new = torch.transpose(fc_batch, 0, 1)
        for i in range(0, sub_num):
            fcd_temp = torch_corr(fc_batch_new[:, :, i])
            fcd_temp_vec = fcd_temp[fcd_mask]
            fcd_hist[:, i + b * sub_num] = torch.histc(fcd_temp_vec, 10000,
                                                       (0.0001 - 1), 1)
    ...
    if resid_num != 0:
        ...
        fc_batch_new = torch.transpose(fc_batch, 0, 1)
        for i in range(0, resid_num):
            fcd_temp = torch_corr(fc_batch_new[:, :, i])
            fcd_temp_vec = fcd_temp[fcd_mask]
            fcd_hist[:, i + sub_num * batch_num] = torch.histc(
                fcd_temp_vec, 10000, (0.0001 - 1), 1)
...
def CBIG_FCDKSstat_single_upgrade(emp_ks, bold_d, n_dup, window_size=83):
    '''
    This function is to calculate the FCD KS statistics cost for multiple simulation
    BOLD signal results
    Args:
...
def CBIG_combined_cost_train(parameter, n_dup, subject_path):
    '''
    ...
    emp_fcd = sio.loadmat(subject_path + 'FCD_train.mat')
    emp_fcd = np.array(emp_fcd['train_FCD'])
    ...
    emp_fc = csv_matrix_read(subject_path + 'FC_train.csv')
    ...
    fcd_cost = CBIG_FCDKSstat_multi_upgrade(emp_fcd, bold_d, n_dup)
...
def CBIG_combined_cost_validation(parameter, n_dup, subject_path):
    '''
    This function is implemented to calcualted the FC correlation and FCD KS
    ...
    emp_fcd = sio.loadmat(subject_path + 'FCD_validation.mat')
    emp_fcd = np.array(emp_fcd['validation_FCD'])

    sc_mat_raw = csv_matrix_read('../../../input/Desikan_input/sc_test.csv')
    sc_mat = sc_mat_raw / sc_mat_raw.max() * 0.2
    sc_mat = torch.from_numpy(sc_mat).type(torch.FloatTensor).cuda()

    emp_fc = csv_matrix_read(subject_path + 'FC_validation.csv')
    emp_fc = torch.from_numpy(emp_fc).type(torch.FloatTensor).cuda()
...
def CBIG_combined_cost_test(parameter, n_dup, subject_path):
    '''
    This function is implemented to calcualted the FC correlation and FCD KS
    ...
    emp_fcd = sio.loadmat(subject_path + '/FCD_test.mat')
    ...
    emp_fc = csv_matrix_read(subject_path + 'FC_test.csv')
    emp_fc = torch.from_numpy(emp_fc).type(torch.FloatTensor).cuda()
    ...
    # Calculating FCD KS statistics cost
    fcd_cost = CBIG_FCDKSstat_single_upgrade(emp_fcd, bold_d, n_dup)
