...
def CBIG_mfm_optimization_desikan_main(gpu_index=0, random_seed=1):
    ...
    start_point_i, template_mat = get_init(
        myelin_data, gradient_data, highest_order, init_para[1:n_node + 1])
    start_point_s, template_mat = get_init(
        myelin_data, gradient_data, highest_order, init_para[n_node + 2:])

    # Initializing childrens
    xmean = np.zeros(N)
    xmean[0:1] = init_para[0]
    xmean[1:(2 * highest_order + 1) + 1] = start_point_i
    ...
    D = np.zeros(N)
    D[0:1] = init_para[0] / 2
    D[1:(2 * highest_order + 1) + 1] = start_point_i[0] / 2
    ...
    while countloop < maxloop:
        ...
        while j < Lambda:
            ...
            input_para[0, j] = arx[0, j]
            input_para[1:n_node + 1, j] = template_mat @ arx[1:(
        ...
        input_para_m = np.zeros((68 * 3 + 1, input_para.shape[1]))
        input_para_m[0:68, :] = np.repeat(input_para[0:1, :], 68, 0)
        input_para_m[68:68 * 3 + 1, :] = input_para[1:dim, :]
