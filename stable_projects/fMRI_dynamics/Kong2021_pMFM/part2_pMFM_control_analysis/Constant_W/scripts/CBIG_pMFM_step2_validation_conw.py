...
def CBIG_mfm_validation_desikan_main(gpu_index=0):
    ...
    for i in range(1, 11):
        ...
        para_w = np.repeat(x_mass[0:1, :], n_node, 0)
        para_I = template_mat @ x_mass[1:2 * highest_order + 2, :]
        para_sigma = template_mat @ x_mass[2 * highest_order + 3:, :]
        arx_mass = np.concatenate(
            (para_w, para_I,
             x_mass[2 * highest_order + 2:2 * highest_order + 3, :],
             para_sigma), 0)
