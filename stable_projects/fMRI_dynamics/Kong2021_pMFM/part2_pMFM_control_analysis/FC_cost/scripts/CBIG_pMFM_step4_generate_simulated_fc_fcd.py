...
def CBIG_pMFM_generate_simualted_fc_fcd(gpu_index=0):
    ...
    fc_sum = np.zeros((n_nodes, n_nodes))
    count = 0
    for i in range(bold_d.shape[0]):
        if bold_d[0, i, -1] == bold_d[0, i, -1]:
            sim_fc = fc.torch_corr(bold_d[:, i, :])
            sim_fc_numpy = sim_fc.cpu().numpy()
            fc_sum += sim_fc_numpy
            count += 1
    sim_fc_ave = fc_sum / count
    FC = {'FC_mat': sim_fc_ave}
    ...
    for i in range(n_set):
        ...
        if count > 10:
            break
    sio.savemat(os.path.join(output_path, 'fc_ave_fccost.mat'), FC)
