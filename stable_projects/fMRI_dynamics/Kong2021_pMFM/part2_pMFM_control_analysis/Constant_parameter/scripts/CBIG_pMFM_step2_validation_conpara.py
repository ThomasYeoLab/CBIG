...
def CBIG_mfm_validation_desikan_main(gpu_index=0,
    ...
    for i in range(1, 11):
        ...
        arx_mass = np.zeros((205, x_mass.shape[1]))
        arx_mass[0:n_node, :] = np.repeat(x_mass[0:1, :], n_node, 0)
        arx_mass[n_node:2 * n_node, :] = np.repeat(x_mass[1:2, :], n_node, 0)
        arx_mass[2 * n_node, :] = x_mass[2, :]
        arx_mass[2 * n_node + 1:, :] = np.repeat(x_mass[3:4, :], n_node, 0)
...
if __name__ == '__main__':
    ...
    CBIG_mfm_validation_desikan_main(0)
