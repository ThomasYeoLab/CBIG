...
def CBIG_mfm_optimization_desikan_main(gpu_index=0, random_seed=1):
    '''
    This function is to implement the optimization processes of mean field
    model.
    The objective function is the summation of FC correlation cost and FCD KS
    statistics cost.
    The optimization process is highly automatic and generate 500 candidate
    parameter sets for main results.
    ...
    while countloop < maxloop:
        ...
        while j < Lambda:
            ...
            input_para[n_node:2 * n_node, j] = \
                template_mat @ arx[2 * highest_order + 1:
                                   2 * (2 * highest_order + 1), j]
            input_para[2 * n_node:2 * n_node +
                       1, j] = arx[2 * (2 * highest_order + 1), j]
            input_para[2 * n_node + 1:dim, j] = \
                template_mat @ arx[2 * (2 * highest_order + 1) + 1:N, j]
        ...
        ps = (1 - cs) * ps + np.sqrt(cs * (2 - cs) * mueff) * \
            np.dot(invsqrtC, xshow) / sigma
        hsig = (np.linalg.norm(ps) / np.sqrt(1 - (1 - cs)**
                                             (2 * countloop)) / chiN <
                (1.4 + 2 / (N + 1))) * 1
        pc = (1 - cc) * pc + hsig * \
            np.sqrt(cc * (2 - cc) * mueff) * xshow / sigma
    ...
    save_name = [output_path] + ['example_output.csv']
