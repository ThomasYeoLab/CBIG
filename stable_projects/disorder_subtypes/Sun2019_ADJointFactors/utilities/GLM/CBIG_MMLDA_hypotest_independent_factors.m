function baseline = CBIG_MMLDA_hypotest_independent_factors(k, N, betas, stats, output, CI_scale)
% baseline = CBIG_MMLDA_hypotest_independent_factors(k, N, betas, stats, output, CI_scale)
%
...
%
% Example:
%   baseline = CBIG_MMLDA_hypotest_independent_factors(3, 100, betas, stats, './')
%
...
switch k
    case 2
        ...
        idx2 = find(H==-1)
        baseline = [H*betas...
            CI_scale*sqrt(stats.covb(idx1, idx1)+stats.covb(idx2, idx2)-2*stats.covb(idx1, idx2)) p];
            
    case 3
        % Overall (F2 - F1 = 0; F3 - F1 = 0)
        H = [0 -1 1 0 zero_nuisance; 0 -1 0 1 zero_nuisance];
        c = [0; 0]; % H*b = c
        p = linhyptest(betas, stats.covb, c, H, stats.dfe);
    ...
    case 4
        ...
        H = [0 -1 1 0 0 zero_nuisance; 0 -1 0 1 0 zero_nuisance; 0 -1 0 0 1 zero_nuisance];
        c = [0; 0; 0]; % H*b = c
        p = linhyptest(betas, stats.covb, c, H, stats.dfe);
