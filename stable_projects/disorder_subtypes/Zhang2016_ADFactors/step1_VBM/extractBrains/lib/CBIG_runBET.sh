...
Usage: $0 -i <imgList> [-n <nameList>] -p <BETParam> -o <outDir> [-q <queue>]
    - imgList   Text file with each line pointing to a NIfTI volume; 
                e.g., ../replicatePNAS/imgList_BETParam_bl/imgList1.txt
    - nameList  (Optional) text file with the same number of lines as imgList. 
                Each line specifies a shorter (or more meaningful) name of the corresponding image. 
                If not provided, output names will be built upon the original filenames
    ...
    - queue     (Optional) if you have a cluster, use it to specify the queue to which you want to qsub these jobs. 
                If not provided, jobs will run serially
" 1>&2; exit 1; }
...
for img in `cat ${imgList}`; do
    ...
    if [ -z "${queue}" ]; then
        ...
        qsub -q ${queue} -v img=${img},name=${name},\
            outDir=${outDir} -o ${logDir}${name}.out -e ${logDir}${name}.err ${BETParam}  
    fi
