...
Usage: $0 -g <GMList> -t <affineTmp> -o <outDir> [-q <queue>]
    ...
    - affineTmp     Study-specific affine template created by the previous step; 
                    e.g., ~/outputs/VBM/affineTmp/affineTmp.nii.gz
...
for GM in `cat ${GMList}`; do
    ...
    if [ -z "${queue}" ]; then
        ...
        logDir=${outDir}logs/
        mkdir -p ${logDir}
        qsub -q ${queue} -v affineTmp=${affineTmp},GM=${GM},filename=${filename},outDir=${outDir} \
            -o ${logDir}${filename}.out -e ${logDir}${filename}.err ./CBIG_step4_nonlinRegToAffineTmp_job.sh
    fi
done
