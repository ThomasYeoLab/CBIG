...
Usage: $0 -g <GMList> -t <nonlinTmp> -o <outDir> [-q <queue>]
    - GMList        Text file with each line being the path to a GM image; 
                    e.g., ~/outputs/VBM/GM/GMList.txt (note that all GM images are included)
    - nonlinTmp     Study-specific nonlinear template created by the previous step; 
                    e.g., ~/outputs/VBM/nonlinTmp/nonlinTmp.nii.gz
...
for GM in `cat ${GMList}`; do
    ...
    if [ -z "${queue}" ]; then
        ...
        logDir=${outDir}logs/
        mkdir -p ${logDir}
        qsub -q ${queue} -v nonlinTmp=${nonlinTmp},GM=${GM},filename=${filename},outDir=${outDir} \
            -o ${logDir}${filename}.out -e ${logDir}${filename}.err ./CBIG_step6_nonlinRegToNonlinTmp_job.sh
    fi
done
