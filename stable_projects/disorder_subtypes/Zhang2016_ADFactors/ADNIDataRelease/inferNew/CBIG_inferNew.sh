...
Usage: $0 -b <brainList> -n <nuisanceVars> -k <K> -o <outDir> [-q <queue>]
    ...
    - queue         (Optional) if you have a cluster, use it to specify the queue to which you want to qsub these jobs; 
                    if not provided, jobs will run serially (potentially very slow!)
" 1>&2; exit 1; }
...
# Line order of nuisanceVars corresponds to that of brainList, but not to that of concatOrder, 
# which depends on the completion order of cluster jobs. Hence, we should reorder lines of nuisanceVars 
# so that it and concatOrder correspond
matlab -nodisplay -nosplash -r "brainList = '${brainList}'; nuisanceVars = '${nuisanceVars}'; \
    concatOrder = '${concatOrder}'; CBIG_reorderNuisance(brainList, nuisanceVars, concatOrder); exit;"
# Now the real business
matlab -nodisplay -nosplash -r "cd ../../step2_LDA/lib/; vol4D = '${vol4D}'; concatOrder = '${concatOrder}'; \
    GMMask = '${GMMask}'; params = '${params}'; nuisanceVars = '${nuisanceVars}'; \
    nuisanceVars = [nuisanceVars(1:end-4) '_matchConcatOrder.csv']; outDir = '${outDir_step2}'; \
    CBIG_brain2doc(vol4D, concatOrder, GMMask, params, nuisanceVars, outDir); exit;"
