function [h,pValue,stat,cValue] = CBIG_lratiotest(uLL,rLL,dof,alpha)

% [h,pValue,stat,cValue] = CBIG_lratiotest(uLL,rLL,dof,alpha)
%LRATIOTEST Likelihood ratio test of model specification
%
% Syntax:
%
%   [h,pValue,stat,cValue] = CBIG_lratiotest(uLL,rLL,dof)
%   [h,pValue,stat,cValue] = CBIG_lratiotest(uLL,rLL,dof,alpha)
%
% Description:
%
...
%   [h,pValue,stat,cValue] = CBIG_lratiotest(uLL,rLL,1)
% 
...
if (nargin < 3) || isempty(uLL) || isempty(rLL) || isempty(dof)
    ...
    error(message('econ:CBIG_lratiotest:UnspecifiedInput'))
...
if ~isvector(uLL) || ~isvector(rLL)
    ...
    error(message('econ:CBIG_lratiotest:LLNonVector'))
    ...
    elseif ~isscalar(uLL) && ~isscalar(rLL)
        if length(uLL) ~= length(rLL)
            ...
            error(message('econ:CBIG_lratiotest:LLLengthMismatch'))
...
if any(rLL > uLL)
    ...
    warning(message('econ:CBIG_lratiotest:RLLExceedsULL'))
...
if ~isvector(dof)
    ...
    error(message('econ:CBIG_lratiotest:DofNonVector'))
      
elseif any(mod(dof,1) ~= 0) || any(dof <= 0)
    
    error(message('econ:CBIG_lratiotest:DofNonPositiveInteger'))
          
elseif ~isscalar(dof) && (length(dof) ~= numTests)
    
    error(message('econ:CBIG_lratiotest:DofLengthMismatch'))
...
if (nargin >= 4) && ~isempty(alpha)
    ...
    if ~isvector(alpha)
        ...
        error(message('econ:CBIG_lratiotest:AlphaNonVector'))
                    
    elseif any(alpha <= 0) || any(alpha >= 1)
        
        error(message('econ:CBIG_lratiotest:AlphaOutOfRange'));
        
    elseif ~isscalar(alpha) && (length(alpha) ~= numTests)
        
        error(message('econ:CBIG_lratiotest:AlphaLengthMismatch'));
