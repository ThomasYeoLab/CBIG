function [betas, stats] = CBIG_fitGLM_2t(subInfoFile, GMICVFile, q)

% [betas, stats] = CBIG_fitGLM_2t(subInfoFile, GMICVFile, q)
...
% Construct X
X = zeros(size(RID_time_score, 1), 10);
for idx = 1:size(X, 1)
    ...
    % Fill in one row
    X(idx, :) = [m d c m*c d*c a g e i y2];
end
