function [betas, stats] = CBIG_fitGLM_4t(subInfoFile, GMICVFile, q)

% [betas, stats] = CBIG_fitGLM_4t(subInfoFile, GMICVFile, q)
...
% Construct X
X = zeros(size(RID_time_score, 1), 15);
for idx = 1:size(X, 1)
    ...
    % Fill in one row
    X(idx, :) = [m d s f p m*s m*f m*p d*s d*f d*p a g e i];
end
