function [results,prams]=CBIG_gwMRF_build_data_and_perform_clustering(input_fullpaths,output_path,start_idx,end_idx,num_left_cluster,num_right_cluster,smoothcost,num_iterations,num_runs,start_gamma,exponential)

    % This function reads in the data, computes the multiplication matrix and performs the clustering
    % Note that reading in the data and building the matrices might take some time and will need some disk space and memory
    % If the matrix files have been already created(and still exist) this process will be skiped the next time
    % This function does not give you access to all the parameters of test_graph_clustering. You will need to call test_graph_cut_clustering directly to have all options. See also alex_set_prams.m for parameters
    %
    % Input:
    ...
    %
    % Example:
    %   - [results,prams]=CBIG_gwMRF_build_data_and_perform_clustering('<your_input_fullpaths_file>','<your_output_path>',1,2,50,50,5000,7,2,50000000,15);
    %
    ...
    if ~(exist([output_path,'/mult_mat/rh_mult_matrix.mat'], 'file') >0)
        [lh_output_file,rh_output_file]=CBIG_gwMRF_build_time_matrix(input_fullpaths,[output_path,'/time_data/'],start_idx,end_idx,fsaverage,'lh_time_matrix.mat','rh_time_matrix.mat');
    ...
    [prams,results]=CBIG_gwMRF_graph_cut_clustering('lh_avg_file',lh_output_mult_mat_file,'rh_avg_file',rh_output_mult_mat_file,'left_cluster',num_left_cluster,'right_cluster',num_right_cluster,'iterations',num_iterations,'smoothcost',smoothcost,'start_index',start_idx, 'runs', num_runs,'output_folder' ,[output_path,'/clustering/'],'dim',dim,'exponential',exponential,'start_gamma',start_gamma);
