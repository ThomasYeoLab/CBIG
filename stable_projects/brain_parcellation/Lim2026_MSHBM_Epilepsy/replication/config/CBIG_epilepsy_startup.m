...
% Modified by Mervyn Lim Jun Rui
...
rmpath(genpath(fullfile(getenv('CBIG_CODE_DIR'), 'utilities', 'matlab')));
rmpath(fullfile(getenv('FREESURFER_HOME'), 'fsfast', 'toolbox'));
rmpath(fullfile(getenv('FREESURFER_HOME'), 'matlab'));
CBIG_SD_DIR = getenv('CBIG_SD_DIR');
if (~isempty(CBIG_SD_DIR) && exist(CBIG_SD_DIR, 'dir'))
    rmpath(genpath(CBIG_SD_DIR));
end
clear CBIG_SD_DIR;
