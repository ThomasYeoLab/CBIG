...
DESCRIPTION:
	This function calls the matlab function '"'CBIG_LiGSR_LME_workflowGSP.m'"' to perform variance component model for 
...
REQUIRED ARGUMENTS:
	...
	-trait_list      trait_list     : The full path to a text file of trait names. Each line in this file corresponds 
	                                  to one trait. The trait names should exist as headers in '"'data_csv'"'.
	...
	-FD_file         FD_file        : The full path to a text file of the mean framewise displacement (FD) of all 
	                                  subjects. The number of lines in '"'FD_file'"' should be the same as the number 
	                                  of lines in '"'sub_list'"'. If the user wants to regress out FD, then the 
	                                  '"'covariate_list'"' should include the string '"'FD'"'. If the covariates do not 
	                                  include FD, this input variable is not needed. The user can pass in NONE.
	-DVARS_file      DVARS_file     : The full path to a text file of the mean DVARS of all subjects. The number of 
	                                  lines in '"'DVARS_file'"' should be the same as the number of lines in 
	                                  '"'subject_list'"'. If the user wants to regress out DVARS, then the 
	                                  '"'covariate_list'"' should include the string '"'DVARS'"' (or '"'DV'"'). If the 
	                                  covariates do not include DVARS, this input variable is not needed. The user can 
	                                  pass in NONE.
...
OPTIONAL ARGUMENTS:
	...
	-ystem           ystem          : The trait values will be read from '"'data_csv'"' and saved in a .mat file: 
	                                  \${outdir}/y_\${ystem}.mat. For example, if '"'trait_list'"' contains 23 
	                                  behavioral names, you can set ystem = 23behaviors.
