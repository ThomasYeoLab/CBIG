...
matlab -nodesktop -nosplash -nodisplay -r " addpath $root_dir; CBIG_LiGSR_KRR_workflowGSP( \
   ...
   '$DVARS_file', '$outdir', '$outstem', '$num_test_folds', '$num_inner_folds', '$seed', '$with_bias' ); \
   exit; " >> $LF 2>&1
...
DESCRIPTION:
	This function calls the matlab function '"'CBIG_LiGSR_KRR_workflowGSP.m'"' to perform kernel ridge regression for 
...
REQUIRED ARGUMENTS:
	...
	-covariate_list    covariate_list   : A text list of covariate names (e.g. age, sex, FD) that need to be regressed 
	                                      ...
	                                      covariate name. Except for FD and DVARS, the covariate name should correspond 
	                                      to one of the headers stated in \"data_csv\".
...
OPTIONAL ARGUMENTS:
	...
	-with_bias         with_bias        : 0 or 1.
	                                      ...
	                                      - with_bias = 1 (or '1') means the algorithm is to minimize
	                                        (y - K*alpha - beta)^2 + (regularization of alpha), where beta is a constant
	                                        bias for every subject, estimated from the data.
...
if [ "$with_bias" == "" ]; then
	echo "ERROR: the flag with_bias not specified"
fi
