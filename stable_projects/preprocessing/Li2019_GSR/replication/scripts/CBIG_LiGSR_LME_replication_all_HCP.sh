...
test_dir=$CBIG_REPDATA_DIR/stable_projects/preprocessing/Li2019_GSR/VarianceComponentModel/HCP
...
for pipeline in GSR Baseline ; do
    ...
    echo $cmd | $CBIG_SCHEDULER_DIR/qsub -V -q circ-spool -l walltime=01:00:00,mem=4GB,nodes=1:ppn=5 -m ae -N \
      CBIG_LiGSR_LME_replication_all_HCP
    sleep 3s
    ...
    echo $cmd | $CBIG_SCHEDULER_DIR/qsub -V -q circ-spool -l walltime=00:30:00,mem=4GB,nodes=1:ppn=3 -m ae -N \
    CBIG_LiGSR_LME_replication_all_HCP
    sleep 3s
