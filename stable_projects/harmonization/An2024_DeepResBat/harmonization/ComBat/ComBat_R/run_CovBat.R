...
ComBatHarm <- function(working_dir, fold_input_path, fold_output_path, train_val_file, test_file, isRef, isSaveTrain){
  ...
  source('neuroCombat/R/neuroCovbat.R');
  library(matrixStats);
  ###################### Training & Validation ######################
  ...
  # run CovBat
  if (isRef == 1){
    train_harm <- neuroCovbat(dat=train_roi, batch=train_site, mod=train_cov, ref.batch = 0)}
  else{
    train_harm <- neuroCovbat(dat=train_roi, batch=train_site, mod=train_cov, ref.batch = NULL)}
  
  # save harmonized training & validation data
  if (isSaveTrain == 1){
    train_harm_ROI = train_harm$dat.covbat
  ...
  test_harm <- neuroCovbatFromTraining(
    test_roi, test_site, estimates = train_harm$dat.estimates, 
    pca_est = train_harm$dat.pca_est,
    npc = train_harm$dat.npc,
    covcomb_est = train_harm$dat.covcomb_est,
    mod = test_cov)
