...
def predict(args):
    ...
    for fold in range(args.nb_folds):
        ...
        train_pkl = load_pkl(
            os.path.join(fold_data_path,
                         'unmatch2match_train' + args.sufix + '.pkl'))
        mean = train_pkl['mean'][ROIs].values
        std = train_pkl['std'][ROIs].values
        for harm_file in args.harm_files:
            harm_pkl = load_pkl(
                os.path.join(fold_data_path, harm_file + args.sufix + '.pkl'))
            ...
            for i in range(args.nb_pred):
                x_hat += \
                    vae_harm_predict(model, x, i, recon=True) / args.nb_pred
                x_hat_map2ADNI += \
                    vae_harm_predict(
                        model, x, i, site=sites_map2ADNI) / args.nb_pred
                x_hat_intermediate += \
                    vae_harm_predict(
                        model, x, i, site=sites_intermediate) / args.nb_pred
            # denormalization
            ...
            map2ADNI_save_name = \
                'harm_' + harm_file + '_ROI-map2ADNI' + args.sufix + '.csv'
            ...
            intermediate_save_name = \
                'harm_' + harm_file + '_ROI-intermediate' + args.sufix + '.csv'
            ...
            save_name = \
                'harm_' + harm_file + '_ROI-recon' + args.sufix + '.csv'
    ...
    replace_with_harmed_ROI_wrapper(
        ...
        args.nb_folds, '-recon' + args.sufix)
    # map2ADNI
    replace_with_harmed_ROI_wrapper(
        os.path.join(args.raw_data_path, args.dataset_pair),
        os.path.join(args.harm_output_path, args.model_name,
                     args.dataset_pair), args.origin_files, args.harm_files,
        args.nb_folds, '-map2ADNI' + args.sufix)
    # intermediate
    replace_with_harmed_ROI_wrapper(
        os.path.join(args.raw_data_path, args.dataset_pair),
        os.path.join(args.harm_output_path, args.model_name,
                     args.dataset_pair), args.origin_files, args.harm_files,
        args.nb_folds, '-intermediate' + args.sufix)
