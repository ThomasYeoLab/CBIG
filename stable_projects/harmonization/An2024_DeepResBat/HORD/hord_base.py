...
class OptimBase(object):
    ...
    def __init__(self, executable, fixed_args, log_file):
        ...
        assert not os.path.isfile(
            self.log_file), '%s exits. Choose a different name' % self.log_file
        ...
        with open(self.log_file, 'w') as fhandler:
            header = ['bestScore', 'score', 'nb_eval'] + list(
                self.hyper_map.keys())
    ...
    def log(self, result, nb_eval, var_args):
        row = [self.best_result, result, nb_eval]
