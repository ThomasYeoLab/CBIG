...
DATA_DIR=$CBIG_REPDATA_DIR/stable_projects/registration/Wu2017_RegistrationFusion/
...
main(){
  ...
  cmd="$RF_VOL2SURF_DIR/CBIG_RF_step2B_RF_ANTs_vol2fsaverage_proj.sh -p MNI152_orig -n 5 -i $output_dir/index_MNI152 "
  cmd="$cmd -w $DATA_DIR/warps -o $output_dir -a $ANTs_dir"
  echo $cmd
  eval $cmd

  # Step 3: Generate average mapping
  cmd="$RF_VOL2SURF_DIR/CBIG_RF_step3_compute_vol2fsaverage_avgMapping.sh -p MNI152_orig -i "
  cmd="$cmd $output_dir/index_fsaverage -n 5 -o $output_dir/mapping -c 0"
  echo $cmd
  eval $cmd
  ...
  cmd="$RF_DIR/bin/scripts_final_proj/CBIG_RF_projectVol2fsaverage_batch.sh -l $output_dir/temp.csv -n 5 -d "
  cmd="$cmd $output_dir/mapping -o $output_dir/projected_vol2fsaverage"
  echo $cmd
  eval $cmd
  ...
  matlab -nodesktop -nojvm -nosplash -r "addpath('$SCRIPT_DIR'); CBIG_RF_replication_compare(\
  '$output_dir/projected_vol2fsaverage/lh.prob_map_central_sulc.5Sub_RF_ANTs_MNI152_orig_to_fsaverage.nii.gz', \
  '$DATA_DIR/data/lh.projected_central_sulc.nii.gz'); rmpath('$SCRIPT_DIR'); exit"
...
This script generates an RF-ANTs MNI-to-fsaverage mapping averaged across 5 subjects and use it to project a 
probabilistic map. 

The projected map should be compared to $DATA_DIR/data/lh.projected_central_sulc.nii.gz and 
