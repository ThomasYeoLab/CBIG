...
main(){
  ...
  cmd="$RF_SURF2VOL_DIR/CBIG_RF_step2B_RF_ANTs_fsaverage2vol_proj.sh -p MNI152_orig -s FSL_MNI152_FS4.5.0 -n 5 -i "
  cmd="$cmd $output_dir/index_fsaverage -w $DATA_DIR/warps -o $output_dir -a $ANTs_dir"
  echo $cmd
  eval $cmd

  # Step 3: Generate average mapping
  cmd="$RF_SURF2VOL_DIR/CBIG_RF_step3_compute_fsaverage2vol_avgMapping.sh -s FSL_MNI152_FS4.5.0 -i "
  cmd="$cmd $output_dir/index_FSL_MNI152_FS4.5.0 -n 5 -o $output_dir -c 0"
  echo $cmd
  eval $cmd
  ...
  cmd="$RF_DIR/bin/scripts_final_proj/CBIG_RF_projectfsaverage2Vol_batch.sh -l $output_dir/temp.csv -n 5 -d "
  cmd="$cmd $output_dir/mapping -o $output_dir/projected_fsaverage2vol"
  echo $cmd
  eval $cmd
  ...
  '$output_dir/projected_fsaverage2vol/surface_parcel.5Sub_fsaverage_to_FSL_MNI152_FS4.5.0_RF_ANTs.nii.gz', \
  '$DATA_DIR/data/projected_surface_parcel.nii.gz'); rmpath('$SCRIPT_DIR'); exit"
...
This script generates an RF-ANTs fsaverage-to-MNI mapping averaged across 5 subjects and use it to project a 
