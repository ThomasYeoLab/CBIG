...
# This script projects xyz index files in a specified number of subjects' surface space to the subjects' T1 space, and 
# then to the volumetric atlas space
  ...
  do
    ...
    do
      ...
      do
        ...
        if [ ! -e $output ]; then
          cmd="CBIG_antsApplyReg_vol2vol.sh -i $input -r $template -d $warp_dir -w $warp_prefix -o "
          cmd="$cmd $output_dir/index_$template_sub_id -p $output_prefix -s forward -t NearestNeighbor -a $ANTs_dir; "
          cmd="$cmd $UTILITIES_DIR/CBIG_RF_convert_orig2norm.sh $output $template $template_norm "
          cmd="$cmd $output_dir/index_$template_sub_id $output_prefix"
...
#usage
usage() { echo "
Usage: $0 -p <template_type> -n <num_of_sub> -a <ants_dir> -i <input_dir> -w <warp_dir> -t <template> -l \
       <ind_sub_list> -g <ind_sub_dir> -o <output_dir> -q <queue> -t <interval>

This script projects existing x/y/z index files in individual subject's surface space to a volumetric atlas space as 
step 2 in RF-ANTs approach. The index files are projected to each subject's own volumetric space. Then they are 
registered to the the volumetric atlas space using ANTs registration results (see $ANTSREG_PREP_SCRIPT about how to run 
ANTs registration).
