...
# This script project xyz index files in a specified number of subjects' surface space to the subject's T1 space, and 
# then to a volumetric atlas space
...
main(){
  ...
  do
    ...
    do
      ...
      cmd="$UTILITIES_DIR/CBIG_RF_proj_fsaverage_to_T1.sh $input_dir $input_prefix $output_dir/index_T1 $ind_sub_dir "
      cmd="$cmd ${sub}"
      echo $cmd
      eval $cmd
      ...
      do
        ...
        if [ ! -e $output ]; then
          cmd="$UTILITIES_DIR/CBIG_RF_talairachM3zReg_vol2vol.sh $ind_sub_dir ${sub} $input $template_sub_dir "
          cmd="$cmd $template_sub_id $output_dir/index_${template_sub_id} $output_prefix nearest"
...
#usage
usage() { echo "
Usage: $0 -i <input_dir> -s <template_sub_id> -n <num_of_sub> -d <template_sub_dir> -l <ind_sub_list> -g <ind_sub_dir> \
       -q <queue> -t <interval> -o <output_dir>

This script projects existing x/y/z index files in individual subject's surface space to a volumetric atlas space as 
step 2 in RF-M3Z approach. The index files are projected to each subject's own volumetric space. Then they are 
registered to the FreeSurfer nonlinear volumetric space and to the volumetric atlas space using FreeSurfer's 
talairach.m3z transform.
...
OPTIONAL ARGUMENTS:
  -n <num_of_sub>    number of subjects to use. This means taking the first <num_of_sub> subjects from 
                      ...
                      <ind_sub_list>.
        [ default: 0 ]
  -i <input_dir>          absolute path to input directory. The inputs are the index files created in step 1, i.e. 
                          the input directory should be the same as output directory in step 1.
...
OUTPUTS:
  ...
  2) index_\${template_sub_id}: 12 files will be generated for each subject, corresponding to the x/y/z index files 
     from left and right hemispheres, registered to FreeSurfer nonlinear volumetric space and to the volumetric atlas 
     space. 
  For example: 
