function [lh_proj_data, rh_proj_data] = CBIG_RF_projectColin2fsaverage(input, interp, lh_map, rh_map)

% Usage: [lh_proj_data, rh_proj_data] = CBIG_RF_ProjectColin2fsaverage(input, interp, lh_map, rh_map)
%
...
%               default is RF-ANTs Colin27-to-fsaverage mapping: 
...
%               for RF-ANTs MNI152-to-fsaverage mapping use: 
%                 $CBIG_CODE_DIR/stable_projects/registration/Wu2017_RegistrationFusion/bin/
...
% EXAMPLE USAGE
% -------------
% >> [lh_proj_data, rh_proj_data] = CBIG_RF_projectColin2fsaverage('Colin_probMap_ants.central_sulc.nii.gz');
...
% >> lh_demo = MRIread('lh.Colin_probMap_ants.central_sulc.demo.nii.gz');
% >> rh_demo = MRIread('rh.Colin_probMap_ants.central_sulc.demo.nii.gz');
...
% >> data = MRIread('Colin_probMap_ants.central_sulc.nii.gz');
% >> data.vol = lh_proj_data;
% >> MRIwrite(data, 'lh.Colin2fsaverage_probMap_ants.central_sulc.nii.gz');
% Then call freeview in terminal:
% freeview -f $FREESURFER_HOME/subjects/fsaverage/surf/lh.inflated:overlay=lh.Colin2fsaverage_probMap_ants.central_sulc.nii.gz
%
% Written by Wu Jianxiao and CBIG under MIT license: https://github.com/ThomasYeoLab/CBIG/blob/master/LICENSE.md

% Function usage
if nargin < 1
    disp('usage: [lh_proj_data, rh_proj_data] = CBIG_RF_projectColin2fsaverage(input, interp, lh_map, rh_map)');
    return
end
...
if nargin < 3
    lh_map = fullfile(dir_uti, 'final_warps_FS5.3', 'lh.avgMapping_allSub_RF_ANTs_Colin27_orig_to_fsaverage.mat');
end
if nargin < 4
    rh_map = fullfile(dir_uti, 'final_warps_FS5.3', 'rh.avgMapping_allSub_RF_ANTs_Colin27_orig_to_fsaverage.mat');
end
