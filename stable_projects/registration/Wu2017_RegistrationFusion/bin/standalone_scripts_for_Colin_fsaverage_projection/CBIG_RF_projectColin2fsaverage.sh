...
main(){
  ...
  $matlab_bin/matlab -nodesktop -nojvm -nosplash -r "addpath('$SCRIPT_DIR'); \
                                         input = MRIread('$input'); \
                                         [lh_proj_data, rh_proj_data] = CBIG_RF_projectColin2fsaverage('$input', '$interp', '$lh_map', '$rh_map'); \
...
OPTIONAL ARGUMENTS:
	-p <template_type>      type of volumetric template used in index files creation. Use 'Colin27_norm' or 'MNI152_norm' for RF-M3Z Colin27-to-fsaverage or MNI152-to-fsaverage mappings, 'Colin27_orig' or 'MNI152_orig' for the respective RF-ANTs mappings.
	                        [ default: Colin27_orig ]
...
EXAMPLE:
	$0 -s $SCRIPT_DIR/Colin_probMap_ants.central_sulc.nii.gz -o \$(pwd)
	freeview -f \$FREESURFER_HOME/subjects/fsaverage/surf/lh.inflated:overlay=lh.Colin_probMap_ants.central_sulc.allSub_RF_ANTs_MNI152_orig_to_fsaverage.nii.gz:overlay=${SCRIPT_DIR}/lh.Colin_probMap_ants.central_sulc.demo.nii.gz:overlay_threshold=0.001,1 -viewport '3d'

	This projects the Colin probabilistic map of central sulcus to fsaverage surface and visualise the left hemisphere results in freeview.
