...
main(){
  # Set-ups
  input_name=Colin_probMap_ants.central_sulc
  input=$SCRIPT_DIR/${input_name}.nii.gz

  # Call vol2fsaverage script
  cmd="$SCRIPT_DIR/CBIG_RF_projectColin2fsaverage.sh -s $input -f $FREESURFER_HOME -m $matlab_bin -o $(pwd)"
  echo $cmd
  eval $cmd

  # Visualise lh results
  output=lh.${input_name}.allSub_RF_ANTs_Colin27_orig_to_fsaverage.nii.gz
...
This script projects an example input in Colin27 space to fsaverage space and visualise the results in freeview. 
...
OUTPUTS:
	$0 will create 2 files, corresponding to the projected data onto fsaverage left and right hemispheres respectively, i.e.:
		lh.MNI_probMap_ants.central_sulc.allSub_RF_ANTs_Colin27_orig_to_fsaverage.nii.gz
		rh.MNI_probMap_ants.central_sulc.allSub_RF_ANTs_Colin27_orig_to_fsaverage.nii.gz
