...
% Written by Gia H. Ngo and CBIG under MIT license: https://github.com/ThomasYeoLab/CBIG/blob/master/LICENSE.md
    ...
    methods (Test)
        function test_Case(testCase)
            ...
            ReferenceDir = fullfile(UnitTestDir, 'ref_output');
            % load reference result
            load(fullfile(ReferenceDir, 'refTheta.mat'));
            
            % load flag
            replace_unittest_flagDir = fullfile(getenv('CBIG_CODE_DIR'), 'unit_tests');
            load(fullfile(replace_unittest_flagDir, 'replace_unittest_flag'));
            ...
            if(replace_unittest_flag)
                disp('Replacing unit test reference results for CBIG_AuthorTopic_unit_test, test_Case...');
                
                disp(['Max theta difference : ' num2str(maxThetaDiff)]);
                refTheta = bestK2Solution.params.theta;
                
                save(fullfile(UnitTestDir, 'ref_output', 'refTheta.mat'), 'refTheta');
