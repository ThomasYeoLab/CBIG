function CBIG_AuthorTopicEM_RunEMwithRandInitExample
  % CBIG_AuthorTopicEM_RunEMwithRandInitExample
  %
  ...
  %    ./EM_outputs_RAND_init/K<K>/alpha100_eta0.01/seed<seed>.mat
  ...
  alpha = 100;
  eta = 0.01;
  seeds = 1:5; % set seeds = 1:1000 to obtain stable estimates

  % run EM inference with random initialization
  em_init_type = 'RAND';
  em_input_path = fullfile(data_dir_path, em_data_file_name);
  disp('Start EM inference');
  for K = all_Ks
    for seed = seeds
      CBIG_AuthorTopicEM_RunEM(seed, K, em_input_path, brain_mask2mm, alpha, eta, em_init_type);
    end
  end
