function dist = ComputeDistBetweenLabels(surface, orig_surface, label1, label2)

% e.g. ComputeDistBetweenLabels('<SUBJECTS_DIR>/<SUBJECT>/surf/lh.sphere', '<SUBJECTS_DIR>/<SUBJECT>/surf/lh.white', 'lateraloccipital_gt', 'lateraloccipital')
% e.g. ComputeDistBetweenLabels('<SUBJECTS_DIR>/<SUBJECT>/surf/lh.sphere', '<SUBJECTS_DIR>/<SUBJECT>/surf/lh.white', '<dir_name>/lh.lateraloccipital_gt.label', '<dir_name>/lh.lateraloccipital.label')
...
if(strcmp(surf_dir, 'surf') == 0)
   error(['ComputeDistBetweenLabels assumes surfaces are in the "surf" directory of the subject, not ' surf_dir]);
end
...
distVec = MARS_computeMeanDistanceBetAnnotationSameSubject(sbjMesh1, sbjMesh1.MARS_label, sbjMesh2.MARS_label);
dist = distVec(2);
