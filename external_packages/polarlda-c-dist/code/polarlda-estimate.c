...
double doc_e_step(document* doc, double* gamma, double** phi, polarlda_model* model, polarlda_suffstats* ss)
{
    double likelihood;
    int n, k;
    double x;
    ...
    likelihood = polarlda_inference(doc, model, gamma, phi);
    ...
    for (n = 0; n < doc->length; n++)
    {
        for (k = 0; k < model->num_topics; k++)
        {
        	x = doc->counts[n] * phi[n][k];
        	ss->class_word_pos[k][doc->words[n]] += x * doc->polarities[n];
        	ss->class_word[k][doc->words[n]] += x;
        	ss->class_total[k] += x;
        }
    }
...
void write_word_assignment(FILE* f, document* doc, double** phi, polarlda_model* model)
{
    int n;
...
/*
 * writes the phi parameters
 *
 */

void write_phi(FILE* f, document* doc, double** phi, polarlda_model* model)
{
    int n, k;
...
void run_em(char* start, char* directory, corpus* corpus)
{
    ...
    polarlda_suffstats* ss = NULL;
    if (strcmp(start, "seeded")==0)
    {
        model = new_polarlda_model(corpus->num_terms, NTOPICS);
        ss = new_polarlda_suffstats(model);
        corpus_initialize_ss(ss, model, corpus);
        polarlda_mle(model, ss, 0);
    ...
    {
        model = load_polarlda_model(start);
        ss = new_polarlda_suffstats(model);
    }

    sprintf(filename,"%s/000",directory);
    save_polarlda_model(model, filename);
    ...
    while (((converged < 0) || (converged > EM_CONVERGED) || (i <= 2)) && (i <= EM_MAX_ITER))
    {
        ...
        polarlda_mle(model, ss, ESTIMATE_ALPHA);
    ...
    FILE* phi_file = fopen(filename, "w");
    for (d = 0; d < corpus->num_docs; d++)
    {
        if ((d % 100) == 0) printf("final e step document %d\n",d);
        likelihood += polarlda_inference(&(corpus->docs[d]), model, var_gamma[d], phi);
        write_word_assignment(w_asgn_file, &(corpus->docs[d]), phi, model);

        // output the phi=Pr(topic | word) 
        write_phi(phi_file, &(corpus->docs[d]), phi, model);
    }
    fclose(w_asgn_file);
...
void infer(char* model_root, char* save, corpus* corpus)
{
    ...
    model = load_polarlda_model(model_root);
    var_gamma = malloc(sizeof(double*)*(corpus->num_docs));
    ...
    for (d = 0; d < corpus->num_docs; d++)
    {
        ...
        likelihood = polarlda_inference(doc, model, var_gamma[d], phi);
...
int main(int argc, char* argv[])
{
    ...
    if (argc == 9) {
        // If a seed is provided, use the provided seed * 2 instead
        t1 = atol(argv[8]) * 2;
    } else {
        (void) time(&t1);
    }
    ...
    {
        printf("usage : polarLDA est [initial alpha] [k] [settings] [data] [random/seeded/*] [directory] [seed]\n");
        printf("        polarLDA inf [settings] [model] [data] [name]\n");
    }
    return(0);
}
