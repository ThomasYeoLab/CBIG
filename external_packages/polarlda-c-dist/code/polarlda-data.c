...
corpus* read_data(char* data_filename)
{
    ...
    while ((fscanf(fileptr, "%10d", &length) != EOF))
    {
       ...
       c->docs[nd].polarities = malloc(sizeof(int)*length);
       for (n = 0; n < length; n++)
       {
           fscanf(fileptr, "%10d:%10d", &word, &count);
           word = word - OFFSET;
           c->docs[nd].words[n] = word;
           c->docs[nd].counts[n] = abs(count);
           c->docs[nd].polarities[n] = (count > 0); // if count>0, polarity=1; else polarity=0
           c->docs[nd].total += abs(count);
