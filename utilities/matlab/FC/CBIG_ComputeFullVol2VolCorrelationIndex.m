function CBIG_ComputeFullVol2VolCorrelationIndex(output_dir, vol_varargin_text, seed_mask_file, vol_mask_file, regress_bool, start_index, stop_index, pval)

% CBIG_ComputeFullVol2VolCorrelationIndex(output_dir, vol_varargin_text, seed_mask_file, vol_mask_file, regress_bool, start_index, stop_index, pval)%
% output_dir:    output directory
% seed_mask_file:      surface mask
% vol_mask_file: volumetric mask
...
for i = min(start_index, length(seed_mask_index)):min(stop_index, length(seed_mask_index))
    ...
    if(~exist([output_dir '/' num2str(x) '_' num2str(y) '_' num2str(z) '.corr.nii.gz'] , 'file'))
...
if(~isempty(seed_mask_index))
    % read volume_mask
    vol_mask = MRIread(vol_mask_file);
    vol_mask_index = find(vol_mask.vol == 1);

    % check vol_mask is a superset of seed mask
    if(~isempty(setdiff(seed_mask_index, vol_mask_index)))
        error('seed_mask_index is not strict subset of vol_mask_index');
    end
    ...
    % This allows indexing into the vol_mask_index
    [tmp, seed_into_vol_index] = intersect(vol_mask_index, seed_mask_index);
    
    % Compute correlation
    for i = 1:length(vol_varargin)
        ...
        for j = 1:length(C3)
            ...
            t_series = single(transpose(reshape(input_series.vol, size(input_series.vol, 1) * size(input_series.vol, 2) * size(input_series.vol, 3), size(input_series.vol, 4))));
            t_series = t_series(:, vol_mask_index);
            ...
            if(regress_bool > 0)
                ...
                if(regress_bool == 1  || regress_bool == 2)
                    for k = 1:length(areas)
                        ...
                        [tmp, regress_target_index] = intersect(vol_mask_index, regress_target);
                    ...
                    error('Does not handle other type of regression for now');
                end
            end

            s_series = t_series(:, seed_into_vol_index);
    ...
    output = output / length(vol_varargin);
    ...
    if(pval > 0)
        ...
        zmat = zmat/length(vol_varargin); % compute mean
        zsqmat = zsqmat/length(vol_varargin);
        stdz = zsqmat - zmat.^2; stdz(stdz < 0) = 0;
        stdz = sqrt(stdz)*length(vol_varargin)/(length(vol_varargin) - 1); %compute std
        clear zsqmat;
        tmat = sqrt(length(vol_varargin))*zmat./(stdz + eps); % compute t stats
        clear stdz; clear zmat;
        pmat = -log10(tcdf(-abs(tmat), length(vol_varargin) - 1)); % compute p val
        clear tmat;
    end
    ...
    for i = 1:length(seed_mask_index)
        ...
        corr_filename = [output_dir '/' num2str(x) '_' num2str(y) '_' num2str(z) '.corr.nii.gz'];
        vol_mask.vol(vol_mask_index) = corr_mat(:, i);
        MRIwrite(vol_mask, corr_filename);

        if(pval > 0)
            prob_filename = [output_dir '/' num2str(x) '_' num2str(y) '_' num2str(z) '.mlog10p.nii.gz'];
            vol_mask.vol(vol_mask_index) = pmat(:, i);
            MRIwrite(vol_mask, prob_filename);
        end
    end
end
