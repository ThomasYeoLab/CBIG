function CBIG_VisualizeSurfaceAnnotationInFreeview(lh_annot_file, rh_annot_file, label, output_dir)
  % CBIG_VisualizeSurfaceAnnotationInFreeview(lh_annot_file, rh_annot_file, label, output_dir)
  %
  % Automatically view the surface annotation of the left and right hemisphere in Freeview, take screenshot of the four views (lateral, medial, dorsal, ventral) of each hemisphere, and stitch them togther into either a grid or serial layout
  %
  % Input:
  %   - lh_annot_file: .annot file containing the surface annotation of the left hemisphere in fsaverage surface space (163842 vertices)
  %   - rh_annot_file: .annot file containing the surface annotation of the right hemisphere in fsaverage surface space (163842 vertices)
  %   - label        : common label of the output images
  %   - output_dir   : directory to store the output images
  % Output:
  %   - Images of the different views of the projected annotation are stored under output_dir. The images are named as '[label]_[hemi]_[view].png', in which [label] is the input argument, [hemi] is either 'lh' or 'rh', [view] is one of the following 'lateral', 'medial', 'dorsal', 'ventral'. A subfolder 'tmp' under output_dir also stores the intermediate files.
  % Example:
  %   CBIG_VisualizeSurfaceAnnotationInFreeview('/data/Work/area8_visualization/area8_lh.annot', '/data/Work/area8_visualization/area8_rh.annot', 'area8', '/data/Work/area8_visualization/output')
  %   The annotation stored in 'area8_lh.annot' and 'area8_rh.annot' will be viewed in FreeView. The output images are stored with the format 'area8_[hemi]_[view].png' under /data/Work/area8_visualization/output'
  %
  ...
  CBIG_ResizeImage(colorscale_file, 80, 0, tmp_colorscale_file);
  ...
  system(CBIG_ViewLHAnnotationInFreeview(lh_annot_file, 0, 0, raw_lh_lateral_file));
  ...
  system(CBIG_ViewLHAnnotationInFreeview(lh_annot_file, 180, 0, raw_lh_medial_file));
  ...
  system(CBIG_ViewLHAnnotationInFreeview(lh_annot_file, 180, 90, raw_lh_dorsal_file));
  ...
  system(CBIG_ViewLHAnnotationInFreeview(lh_annot_file, 180, -90, raw_lh_ventral_file));
  ...
  system(CBIG_ViewRHAnnotationInFreeview(rh_annot_file, 180, 0, raw_rh_lateral_file));
  ...
  system(CBIG_ViewRHAnnotationInFreeview(rh_annot_file, 0, 0, raw_rh_medial_file));
  ...
  system(CBIG_ViewRHAnnotationInFreeview(rh_annot_file, 180, 90, raw_rh_dorsal_file));
  ...
  system(CBIG_ViewRHAnnotationInFreeview(rh_annot_file, 180, -90, raw_rh_ventral_file));
  ...
  CBIG_ResizeImage(dorsal_file, 0, lm_h, tmp_dorsal_file);
  CBIG_ResizeImage(ventral_file, 0, lm_h, tmp_dorsal_file);
  ...
  system(CBIG_InsertColorscaleToImage(tmp_colorscale_file, tmp_series_output_path, series_output_path, 0.58, 0.49, 0.85));
