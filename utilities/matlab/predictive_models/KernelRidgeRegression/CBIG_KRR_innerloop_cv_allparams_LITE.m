function [acc, acc_concat] = CBIG_KRR_innerloop_cv_allparams_LITE( test_fold, sub_fold, num_inner_folds, ...
...
% 
% This is a lite version of CBIG_KRR_innerloop_cv_allparams.m. In this
% version, the kernels are not computed and saved separately for each
% inner-loop fold and each outer training-test fold. Instead, the kernel is
% only computed once across all the subjects, and for each fold, the
% scripts will grab the corresponding entries on the fly.
% 
...
if(~exist(fullfile(outdir, ['acc' y_resid_stem '.mat']), 'file'))
    ...
    for k = 1:length(ker_param)
        ...
        FSM = kernel{k}.FSM(sub_index, sub_index);
        ...
        for l = 1:length(lambda_set)
            ...
            for t = 1:length(threshold_set)
                ...
                [y_p, y_t, acc_fold, acc_concat_fold] = CBIG_KRR_innerloop_cv( bin_flag, num_inner_folds, ...
                    FSM, curr_y_resid, curr_y_orig, num_valid_sub, with_bias, lambda, threshold );
                clear threshold
                acc{k,l,t} = acc_fold;
